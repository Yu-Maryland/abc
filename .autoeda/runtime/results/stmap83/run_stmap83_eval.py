...
RESULTS = ROOT / ".autoeda" / "runtime" / "results" / "stmap83"
ABC = ROOT / "abc"
LIB = "7nm_lvt_ff.lib"
COMMAND = "stmap83"
BENCHMARKS = [
...
STMAP_LINE_RE = re.compile(r"^stmap83 ([^:]+):\s*(.*)$")
...
def parse_stmap83_rows(bench, text):
    rows_by_kind = {}
...
def run_flow(kind, bench):
    ...
    return {
        ...
        "diagnostics": parse_stmap83_rows(bench, text) if kind == "candidate" else {},
