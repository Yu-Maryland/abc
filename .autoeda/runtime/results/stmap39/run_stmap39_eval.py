...
OUT = ROOT / ".autoeda/runtime/results/stmap39"
LIB = "7nm_lvt_ff.lib"
COMMAND = "stmap39"
BENCHMARKS = [
    "benchmarks/i10.aig",
...
def parse_candidate_diags(rows):
    diag = {
        ...
        "feedback_load_stats": [],
        "scl_load_stats": [],
        "consumer_pressure_stats": [],
        "consumer_pressure_top": [],
        "scl_load_top": [],
        "feedback_load_top": [],
    ...
    patterns = {
        "guard_stats": re.compile(r"stmap39 guard stats: exact-risk = (\d+)\s+highest = (\d+)\s+lower-mod = (\d+)\s+upper-mod = (\d+)\s+middle-slack = (\d+)\s+middle-relief = (\d+)\s+reject-slack = (\d+)\s+reject-highest = (\d+)\s+reject-arrival = (\d+)\s+reject-area = (\d+)"),
        "near_miss_stats": re.compile(r"stmap39 near-miss stats: near-miss = (\d+)"),
        "early_seed_stats": re.compile(r"stmap39 early-seed stats: early-seed = (\d+)"),
        "penalty_stats": re.compile(r"stmap39 penalty stats: moderate-penalty-seed = (\d+)\s+moderate-penalty-blocked = (\d+)\s+strong-penalty-seed = (\d+)\s+strong-penalty-blocked = (\d+)"),
        "load_stats": re.compile(r"stmap39 (feedback-load|scl-load) stats: nodes = (\d+)\s+matched = (\d+)\s+fanout-pins = (\d+)\s+over-max = (\d+)\s+avg-load-ratio = ([0-9.]+)\s+max-load-ratio = ([0-9.]+)"),
        "load_top": re.compile(r"stmap39 (feedback-load|scl-load) top: rank = (\d+)\s+node = (\d+)\s+gate = (\S+)\s+fanouts = (\d+)\s+load = ([0-9.]+)\s+max-cap = ([0-9.]+)\s+load-ratio = ([0-9.]+)"),
        "pressure_stats": re.compile(r"stmap39 consumer-pressure stats: roots = (\d+)\s+consumer-fanin = (\d+)\s+consumer-fanout = (\d+)\s+selected = (\d+)\s+capacity = (\d+)\s+tracked-aig = (-?\d+)\s+tracked-node = (-?\d+)\s+tracked-ratio = ([0-9.]+)"),
        "pressure_top": re.compile(r"stmap39 consumer-pressure: rank = (\d+)\s+aig-id = (-?\d+)\s+mapped-node = (-?\d+)\s+consumer-load-ratio = ([0-9.]+)"),
        "moderate_diag": re.compile(r"stmap39 moderate penalty (seed|block) diag: index = (\d+)\s+node = (\d+)\s+aig-id = (-?\d+)\s+level = (\d+)\s+refs = (\d+)\s+leaves = (\d+)\s+phase = (\d+)\s+slack = ([0-9.+-]+)\s+area-save = ([0-9.+-]+)\s+arrival-delta = ([0-9.+-]+)\s+arrival-gain-margin = ([0-9.+-]+)\s+penalty-factor = ([0-9.]+)\s+area-margin = ([0-9.+-]+)"),
        "strong_diag": re.compile(r"stmap39 strong penalty (seed|block) diag: index = (\d+)\s+node = (\d+)\s+aig-id = (-?\d+)\s+level = (\d+)\s+refs = (\d+)\s+leaves = (\d+)\s+phase = (\d+)\s+slack = ([0-9.+-]+)\s+area-save = ([0-9.+-]+)\s+arrival-delta = ([0-9.+-]+)\s+arrival-gain-margin = ([0-9.+-]+)\s+penalty-factor = ([0-9.]+)\s+area-margin = ([0-9.+-]+)\s+cut-leaf-load-avg = ([0-9.]+)\s+fanout-limit = (\d+)\s+load-drive-ratio = ([0-9.]+)\s+node-pressure-ratio = ([0-9.]+)\s+cut-pressure-ratio = ([0-9.]+)\s+scl-feedback = ([0-9.]+)"),
    }
    for row in rows:
        ...
        for match in patterns["pressure_stats"].finditer(text):
            diag["consumer_pressure_stats"].append(dict(zip(["benchmark", "roots", "consumer_fanin", "consumer_fanout", "selected", "capacity", "tracked_aig", "tracked_node", "tracked_ratio"], [bench] + list(match.groups()))))
        for match in patterns["pressure_top"].finditer(text):
            diag["consumer_pressure_top"].append(dict(zip(["benchmark", "rank", "aig_id", "mapped_node", "consumer_load_ratio"], [bench] + list(match.groups()))))
...
def main():
    ...
    fields = {
        ...
        "consumer_pressure_stats": ["benchmark", "roots", "consumer_fanin", "consumer_fanout", "selected", "capacity", "tracked_aig", "tracked_node", "tracked_ratio"],
        "consumer_pressure_top": ["benchmark", "rank", "aig_id", "mapped_node", "consumer_load_ratio"],
