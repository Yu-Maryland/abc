...
RESULTS = ROOT / ".autoeda" / "runtime" / "results" / "stmap94"
ABC = ROOT / "abc"
LIB = "7nm_lvt_ff.lib"
COMMAND = "stmap94"
BENCHMARKS = [
...
STMAP_LINE_RE = re.compile(r"^stmap94 ([^:]+):\s*(.*)$")
...
def parse_stmap94_rows(bench, text):
    rows_by_kind = {}
...
def run_flow(kind, bench):
    ...
    return {
        ...
        "diagnostics": parse_stmap94_rows(bench, text) if kind == "candidate" else {},
