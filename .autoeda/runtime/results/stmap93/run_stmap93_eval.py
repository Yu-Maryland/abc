...
RESULTS = ROOT / ".autoeda" / "runtime" / "results" / "stmap93"
ABC = ROOT / "abc"
LIB = "7nm_lvt_ff.lib"
COMMAND = "stmap93"
BENCHMARKS = [
...
STMAP_LINE_RE = re.compile(r"^stmap93 ([^:]+):\s*(.*)$")
...
def parse_stmap93_rows(bench, text):
    rows_by_kind = {}
...
def run_flow(kind, bench):
    ...
    return {
        ...
        "diagnostics": parse_stmap93_rows(bench, text) if kind == "candidate" else {},
