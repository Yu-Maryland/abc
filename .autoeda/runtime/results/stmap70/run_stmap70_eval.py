...
RESULTS = ROOT / ".autoeda" / "runtime" / "results" / "stmap70"
ABC = ROOT / "abc"
LIB = "7nm_lvt_ff.lib"
COMMAND = "stmap70"
BENCHMARKS = [
...
STAT_PATTERNS = {
    ...
    "final_witness_stats": re.compile(
        r"stmap70 final-witness stats: witnesses = (?P<witnesses>\d+)  final-delay = (?P<final_delay>[0-9]+(?:\.[0-9]+)?)"
    ),
    "final_witness_summary": re.compile(
        r"stmap70 final-witness summary: witnesses = (?P<witnesses>\d+)  matched = (?P<matched>\d+)  criticality-ge-0p50 = (?P<criticality_ge_0p50>\d+)"
    ),
