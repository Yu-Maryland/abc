...
RESULTS = ROOT / ".autoeda" / "runtime" / "results" / "stmap80"
ABC = ROOT / "abc"
LIB = "7nm_lvt_ff.lib"
COMMAND = "stmap80"
BENCHMARKS = [
...
STMAP_LINE_RE = re.compile(r"^stmap80 ([^:]+):\s*(.*)$")
...
def parse_stmap80_rows(bench, text):
    rows_by_kind = {}
...
def run_flow(kind, bench):
    ...
    return {
        ...
        "diagnostics": parse_stmap80_rows(bench, text) if kind == "candidate" else {},
