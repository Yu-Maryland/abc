...
OUT = ROOT / ".autoeda/runtime/results/stmap43"
LIB = "7nm_lvt_ff.lib"
COMMAND = "stmap43"
BENCHMARKS = [
    "benchmarks/i10.aig",
...
def parse_candidate_diags(rows):
    ...
    patterns = {
        "guard_stats": re.compile(r"stmap43 guard stats: exact-risk = (\d+)\s+highest = (\d+)\s+lower-mod = (\d+)\s+upper-mod = (\d+)\s+middle-slack = (\d+)\s+middle-relief = (\d+)\s+reject-slack = (\d+)\s+reject-highest = (\d+)\s+reject-arrival = (\d+)\s+reject-area = (\d+)"),
        "near_miss_stats": re.compile(r"stmap43 near-miss stats: near-miss = (\d+)"),
        "early_seed_stats": re.compile(r"stmap43 early-seed stats: early-seed = (\d+)"),
        "penalty_stats": re.compile(r"stmap43 penalty stats: moderate-penalty-seed = (\d+)\s+moderate-penalty-blocked = (\d+)\s+strong-penalty-seed = (\d+)\s+strong-penalty-blocked = (\d+)"),
        "load_stats": re.compile(r"stmap43 (feedback-load|scl-load) stats: nodes = (\d+)\s+matched = (\d+)\s+fanout-pins = (\d+)\s+over-max = (\d+)\s+avg-load-ratio = ([0-9.]+)\s+max-load-ratio = ([0-9.]+)"),
        "load_criticality": re.compile(r"stmap43 (feedback-load|scl-load) criticality: timing-delay = ([0-9.]+)\s+critical-nodes = (\d+)\s+avg-criticality = ([0-9.]+)\s+max-criticality = ([0-9.]+)"),
        "load_top": re.compile(r"stmap43 (feedback-load|scl-load) top: rank = (\d+)\s+node = (\d+)\s+gate = (\S+)\s+fanouts = (\d+)\s+load = ([0-9.]+)\s+max-cap = ([0-9.]+)\s+load-ratio = ([0-9.]+)"),
        "pressure_stats": re.compile(r"stmap43 sink-pressure stats: roots = (\d+)\s+consumer-fanin = (\d+)\s+consumer-fanout = (\d+)\s+raw-pressure-entries = (\d+)\s+raw-pressure-updates = (\d+)\s+sink-pressure-entries = (\d+)\s+sink-pressure-updates = (\d+)\s+pressure-missed = (\d+)\s+pressure-capacity = (\d+)\s+tracked-aig = (-?\d+)\s+tracked-node = (-?\d+)\s+tracked-raw-ratio = ([0-9.]+)\s+tracked-sink-ratio = ([0-9.]+)"),
        "pressure_top": re.compile(r"stmap43 sink-pressure: rank = (\d+)\s+aig-id = (-?\d+)\s+mapped-node = (-?\d+)\s+sink-pressure-ratio = ([0-9.]+)"),
        "moderate_diag": re.compile(r"stmap43 moderate penalty (seed|block) diag: index = (\d+)\s+node = (\d+)\s+aig-id = (-?\d+)\s+level = (\d+)\s+refs = (\d+)\s+leaves = (\d+)\s+phase = (\d+)\s+slack = ([0-9.+-]+)\s+area-save = ([0-9.+-]+)\s+arrival-delta = ([0-9.+-]+)\s+arrival-gain-margin = ([0-9.+-]+)\s+penalty-factor = ([0-9.]+)\s+area-margin = ([0-9.+-]+)\s+node-sink-pressure-ratio = ([0-9.]+)\s+cut-sink-pressure-ratio = ([0-9.]+)\s+scl-feedback = ([0-9.]+)"),
        "strong_diag": re.compile(r"stmap43 strong penalty (seed|block) diag: index = (\d+)\s+node = (\d+)\s+aig-id = (-?\d+)\s+level = (\d+)\s+refs = (\d+)\s+leaves = (\d+)\s+phase = (\d+)\s+slack = ([0-9.+-]+)\s+area-save = ([0-9.+-]+)\s+arrival-delta = ([0-9.+-]+)\s+arrival-gain-margin = ([0-9.+-]+)\s+penalty-factor = ([0-9.]+)\s+area-margin = ([0-9.+-]+)\s+cut-leaf-load-avg = ([0-9.]+)\s+fanout-limit = (\d+)\s+load-drive-ratio = ([0-9.]+)\s+node-sink-pressure-ratio = ([0-9.]+)\s+cut-sink-pressure-ratio = ([0-9.]+)\s+scl-feedback = ([0-9.]+)"),
    }
    for row in rows:
        ...
        for match in patterns["pressure_stats"].finditer(text):
            diag["sink_pressure_stats"].append(dict(zip(["benchmark", "roots", "consumer_fanin", "consumer_fanout", "raw_pressure_entries", "raw_pressure_updates", "sink_pressure_entries", "sink_pressure_updates", "pressure_missed", "pressure_capacity", "tracked_aig", "tracked_node", "tracked_raw_ratio", "tracked_sink_ratio"], [bench] + list(match.groups()))))
        for match in patterns["pressure_top"].finditer(text):
            diag["sink_pressure_top"].append(dict(zip(["benchmark", "rank", "aig_id", "mapped_node", "sink_pressure_ratio"], [bench] + list(match.groups()))))
        for match in patterns["moderate_diag"].finditer(text):
            ...
            diag[key].append(dict(zip(["benchmark", "index", "node", "aig_id", "level", "refs", "leaves", "phase", "slack", "area_save", "arrival_delta", "arrival_gain_margin", "penalty_factor", "area_margin", "node_sink_pressure_ratio", "cut_sink_pressure_ratio", "scl_feedback"], [bench] + list(match.groups()[1:]))))
        for match in patterns["strong_diag"].finditer(text):
            ...
            diag[key].append(dict(zip(["benchmark", "index", "node", "aig_id", "level", "refs", "leaves", "phase", "slack", "area_save", "arrival_delta", "arrival_gain_margin", "penalty_factor", "area_margin", "cut_leaf_load_avg", "fanout_limit", "load_drive_ratio", "node_sink_pressure_ratio", "cut_sink_pressure_ratio", "scl_feedback"], [bench] + list(match.groups()[1:]))))
    return diag
...
def main():
    ...
    fields = {
        ...
        "sink_pressure_top": ["benchmark", "rank", "aig_id", "mapped_node", "sink_pressure_ratio"],
