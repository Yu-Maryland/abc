...
RESULTS = ROOT / ".autoeda" / "runtime" / "results" / "stmap95"
ABC = ROOT / "abc"
LIB = "7nm_lvt_ff.lib"
COMMAND = "stmap95"
BENCHMARKS = [
...
STMAP_LINE_RE = re.compile(r"^stmap95 ([^:]+):\s*(.*)$")
...
def parse_stmap95_rows(bench, text):
    rows_by_kind = {}
...
def run_flow(kind, bench):
    ...
    return {
        ...
        "diagnostics": parse_stmap95_rows(bench, text) if kind == "candidate" else {},
