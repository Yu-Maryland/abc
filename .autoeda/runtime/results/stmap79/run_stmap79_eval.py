...
RESULTS = ROOT / ".autoeda" / "runtime" / "results" / "stmap79"
ABC = ROOT / "abc"
LIB = "7nm_lvt_ff.lib"
COMMAND = "stmap79"
BENCHMARKS = [
...
STAT_PATTERNS = {
    ...
    "final_witness_stats": re.compile(
        r"stmap79 final-witness stats: witnesses = (?P<witnesses>\d+)  final-delay = (?P<final_delay>[0-9]+(?:\.[0-9]+)?)"
    ),
    "final_witness_summary": re.compile(
        r"stmap79 final-witness summary: witnesses = (?P<witnesses>\d+)  matched = (?P<matched>\d+)  criticality-ge-0p50 = (?P<criticality_ge_0p50>\d+)"
    ),
    "final_critical_stats": re.compile(
        r"stmap79 final-critical stats: nodes = (?P<nodes>\d+)  final-delay = (?P<final_delay>[0-9]+(?:\.[0-9]+)?)  criticality-ge-0p50 = (?P<criticality_ge_0p50>\d+)  criticality-ge-0p75 = (?P<criticality_ge_0p75>\d+)  slack-le-5ps = (?P<slack_le_5ps>\d+)"
    ),
    "selected_match_stats": re.compile(
        r"stmap79 selected-match stats: watched-aigs = (?P<watched_aigs>\d+)  rows = (?P<rows>\d+)  watch0-aig-id = (?P<watch0_aig_id>-?\d+)  watch0-rows = (?P<watch0_rows>\d+)  watch1-aig-id = (?P<watch1_aig_id>-?\d+)  watch1-rows = (?P<watch1_rows>\d+)  watch2-aig-id = (?P<watch2_aig_id>-?\d+)  watch2-rows = (?P<watch2_rows>\d+)  watch3-aig-id = (?P<watch3_aig_id>-?\d+)  watch3-rows = (?P<watch3_rows>\d+)"
    ),
    "phase_survival_stats": re.compile(
        r"stmap79 phase-survival stats: watched-aigs = (?P<watched_aigs>\d+)  matched-aigs = (?P<matched_aigs>\d+)  final-delay = (?P<final_delay>[0-9]+(?:\.[0-9]+)?)"
    ),
    "reconstruct_stats": re.compile(
        r"stmap79 reconstruct stats: watched-aigs = (?P<watched_aigs>\d+)  rows = (?P<rows>\d+)  demands = (?P<demands>\d+)  requests = (?P<requests>\d+)  direct-emits = (?P<direct_emits>\d+)  inverters = (?P<inverters>\d+)  cache-hits = (?P<cache_hits>\d+)  phase0-requests = (?P<phase0_requests>\d+)  phase1-requests = (?P<phase1_requests>\d+)  phase0-direct = (?P<phase0_direct>\d+)  phase1-direct = (?P<phase1_direct>\d+)  phase0-inverters = (?P<phase0_inverters>\d+)  phase1-inverters = (?P<phase1_inverters>\d+)  phase0-cache = (?P<phase0_cache>\d+)  phase1-cache = (?P<phase1_cache>\d+)"
    ),
    "demand_path_stats": re.compile(
        r"stmap79 demand-path stats: watch-aig = (?P<watch_aig>-?\d+)  downstream-aig = (?P<downstream_aig>-?\d+)  rows = (?P<rows>\d+)  target-requests = (?P<target_requests>\d+)  target-phase0 = (?P<target_phase0>\d+)  target-phase1 = (?P<target_phase1>\d+)  target-via-downstream = (?P<target_via_downstream>\d+)  leaf-demands = (?P<leaf_demands>\d+)  leaf-phase0 = (?P<leaf_phase0>\d+)  leaf-phase1 = (?P<leaf_phase1>\d+)  leaf-via-downstream = (?P<leaf_via_downstream>\d+)  phase0-direct-or-cached = (?P<phase0_direct_or_cached>\d+)  phase0-inverter-from-opposite = (?P<phase0_inverter_from_opposite>\d+)"
    ),
    "parent_cut_stats": re.compile(
        r"stmap79 parent-cut stats: child-aig = (?P<child_aig>-?\d+)  parent-aig = (?P<parent_aig>-?\d+)  requests = (?P<requests>\d+)  rows = (?P<rows>\d+)  phase0-has-cut = (?P<phase0_has_cut>\d+)  phase1-has-cut = (?P<phase1_has_cut>\d+)  child-phase0 = (?P<child_phase0>\d+)  child-phase1 = (?P<child_phase1>\d+)  child-missing = (?P<child_missing>\d+)"
    ),
...
EMPTY_STAT_FIELDS = {
    ...
    "reconstruct_stats": [
        "benchmark", "watched_aigs", "rows", "demands", "requests", "direct_emits",
        "inverters", "cache_hits", "phase0_requests", "phase1_requests",
        "phase0_direct", "phase1_direct", "phase0_inverters", "phase1_inverters",
        "phase0_cache", "phase1_cache",
    ],
    "demand_path_stats": [
        "benchmark", "watch_aig", "downstream_aig", "rows", "target_requests",
        "target_phase0", "target_phase1", "target_via_downstream",
        "leaf_demands", "leaf_phase0", "leaf_phase1", "leaf_via_downstream",
        "phase0_direct_or_cached", "phase0_inverter_from_opposite",
    ],
    "parent_cut_stats": [
        "benchmark", "child_aig", "parent_aig", "requests", "rows",
        "phase0_has_cut", "phase1_has_cut", "child_phase0", "child_phase1",
        "child_missing",
    ],
}
EMPTY_MULTI_FIELDS = {
    ...
    "reconstruct_demand": [
        "benchmark", "index", "watch_index", "co_kind", "co_index", "node",
        "aig_id", "requested_phase", "complemented_driver",
    ],
    "reconstruct_request": [
        "benchmark", "index", "watch_index", "node", "aig_id",
        "requested_phase", "has_cut_requested", "has_cut_opposite",
        "cached_requested", "cached_opposite", "level",
    ],
    "reconstruct_cache": [
        "benchmark", "index", "watch_index", "node", "aig_id", "phase",
        "cached_node", "via",
    ],
    "reconstruct_emit": [
        "benchmark", "index", "watch_index", "node", "aig_id", "phase",
        "new_node", "gate", "leaves", "u_phase_best", "fanout_limit",
        "leaf0_aig_id", "leaf0_phase", "leaf1_aig_id", "leaf1_phase",
        "leaf2_aig_id", "leaf2_phase", "leaf3_aig_id", "leaf3_phase",
        "leaf4_aig_id", "leaf4_phase", "leaf5_aig_id", "leaf5_phase",
    ],
    "reconstruct_inverter": [
        "benchmark", "index", "watch_index", "node", "aig_id",
        "requested_phase", "source_phase", "source_node", "source_gate",
        "inverter_node", "inverter_gate",
    ],
    "demand_path": [
        "benchmark", "index", "co_kind", "co_index", "watch_aig",
        "downstream_aig", "requested_phase", "stack_depth", "via_downstream",
        "parent_node", "parent_aig_id", "parent_phase", "parent_is_downstream",
        "phase0_action", "phase1_action", "phase0_has_cut", "phase1_has_cut",
        "phase0_cached", "phase1_cached", "phase0_gate", "phase1_gate",
        "path_aigs", "path_nodes", "path_phases",
    ],
    "demand_leaf": [
        "benchmark", "index", "co_kind", "co_index", "parent_node",
        "parent_aig_id", "parent_phase", "parent_is_downstream", "parent_gate",
        "leaf_index", "leaf_aig_id", "leaf_requested_phase", "leaf_inverted_pin",
        "parent_cut_leaves", "u_phase_best", "path_aigs", "path_nodes",
        "path_phases",
    ],
    "parent_cut": [
        "benchmark", "index", "parent_aig", "child_aig", "phase", "has_cut",
        "gate", "leaves", "u_phase_best", "fanout_limit", "child_leaf_index",
        "child_requested_phase", "child_inverted_pin", "leaf0_aig_id",
        "leaf0_phase", "leaf1_aig_id", "leaf1_phase", "leaf2_aig_id",
        "leaf2_phase", "leaf3_aig_id", "leaf3_phase", "leaf4_aig_id",
        "leaf4_phase", "leaf5_aig_id", "leaf5_phase",
    ],
}
...
def _join_sorted_values(rows, key):
    values = {row.get(key, "") for row in rows if row.get(key, "") != ""}
    return "+".join(sorted(values, key=lambda v: int(v) if v.lstrip("-").isdigit() else v))


def write_reconstruct_summary(stats_by_name, multi_stats_by_name):
    rows_out = []
    for bench in BENCHMARKS:
        stat_rows = [row for row in stats_by_name.get("reconstruct_stats", []) if row["benchmark"] == bench]
        stat = stat_rows[0] if stat_rows else {}
        requests = [row for row in multi_stats_by_name.get("reconstruct_request", []) if row["benchmark"] == bench]
        emits = [row for row in multi_stats_by_name.get("reconstruct_emit", []) if row["benchmark"] == bench]
        inverters = [row for row in multi_stats_by_name.get("reconstruct_inverter", []) if row["benchmark"] == bench]
        caches = [row for row in multi_stats_by_name.get("reconstruct_cache", []) if row["benchmark"] == bench]
        survival = [row for row in multi_stats_by_name.get("phase_survival", []) if row["benchmark"] == bench]
        survival_row = survival[0] if survival else {}
        phase0_emit = next((row for row in emits if row.get("phase") == "0"), {})
        phase1_emit = next((row for row in emits if row.get("phase") == "1"), {})
        phase0_nodes = int(survival_row.get("phase0_nodes", "0") or 0)
        phase1_nodes = int(survival_row.get("phase1_nodes", "0") or 0)
        rows_out.append({
            "benchmark": bench,
            "aig_id": stat.get("watched_aigs", "0") != "0" and (requests[0].get("aig_id", "") if requests else survival_row.get("aig_id", "")) or "",
            "rows": stat.get("rows", ""),
            "requests": stat.get("requests", ""),
            "direct_emits": stat.get("direct_emits", ""),
            "inverters": stat.get("inverters", ""),
            "cache_hits": stat.get("cache_hits", ""),
            "requested_phases": _join_sorted_values(requests, "requested_phase"),
            "direct_phases": _join_sorted_values(emits, "phase"),
            "inverter_requested_phases": _join_sorted_values(inverters, "requested_phase"),
            "cache_phases": _join_sorted_values(caches, "phase"),
            "phase0_direct_gate": phase0_emit.get("gate", ""),
            "phase1_direct_gate": phase1_emit.get("gate", ""),
            "phase0_final_nodes": survival_row.get("phase0_nodes", ""),
            "phase1_final_nodes": survival_row.get("phase1_nodes", ""),
            "both_direct_phases": int(bool(phase0_emit) and bool(phase1_emit)),
            "both_final_phases": int(phase0_nodes > 0 and phase1_nodes > 0),
        })
    write_csv(
        RESULTS / "reconstruct_summary.csv",
        rows_out,
        [
            "benchmark", "aig_id", "rows", "requests", "direct_emits",
            "inverters", "cache_hits", "requested_phases", "direct_phases",
            "inverter_requested_phases", "cache_phases", "phase0_direct_gate",
            "phase1_direct_gate", "phase0_final_nodes", "phase1_final_nodes",
            "both_direct_phases", "both_final_phases",
        ],
    )


def write_demand_path_summary(stats_by_name, multi_stats_by_name):
    rows_out = []
    for bench in BENCHMARKS:
        stat_rows = [row for row in stats_by_name.get("demand_path_stats", []) if row["benchmark"] == bench]
        stat = stat_rows[0] if stat_rows else {}
        paths = [row for row in multi_stats_by_name.get("demand_path", []) if row["benchmark"] == bench]
        leaves = [row for row in multi_stats_by_name.get("demand_leaf", []) if row["benchmark"] == bench]
        downstream_paths = [row for row in paths if row.get("via_downstream") == "1"]
        downstream_leaves = [row for row in leaves if row.get("parent_is_downstream") == "1"]
        first_downstream_leaf = downstream_leaves[0] if downstream_leaves else {}
        first_downstream_path = downstream_paths[0] if downstream_paths else {}
        phase0_inverter_rows = [row for row in paths if row.get("phase0_action") == "inverter-from-opposite"]
        phase0_direct_rows = [row for row in paths if row.get("phase0_action") in ("direct-cut", "cached")]
        rows_out.append({
            "benchmark": bench,
            "watch_aig": stat.get("watch_aig", first_downstream_path.get("watch_aig", "")),
            "downstream_aig": stat.get("downstream_aig", first_downstream_path.get("downstream_aig", "")),
            "target_requests": stat.get("target_requests", ""),
            "target_phase0": stat.get("target_phase0", ""),
            "target_phase1": stat.get("target_phase1", ""),
            "target_via_downstream": stat.get("target_via_downstream", ""),
            "leaf_demands": stat.get("leaf_demands", ""),
            "leaf_phase0": stat.get("leaf_phase0", ""),
            "leaf_phase1": stat.get("leaf_phase1", ""),
            "leaf_via_downstream": stat.get("leaf_via_downstream", ""),
            "phase0_actions": _join_sorted_values(paths, "phase0_action"),
            "phase0_direct_or_cached_rows": len(phase0_direct_rows),
            "phase0_inverter_from_opposite_rows": len(phase0_inverter_rows),
            "downstream_parent_phases": _join_sorted_values(downstream_leaves, "parent_phase"),
            "downstream_leaf_requested_phases": _join_sorted_values(downstream_leaves, "leaf_requested_phase"),
            "output_cos_requesting_watch": _join_sorted_values(paths, "co_index"),
            "output_cos_via_downstream": _join_sorted_values(downstream_paths, "co_index"),
            "first_downstream_parent_gate": first_downstream_leaf.get("parent_gate", ""),
            "first_downstream_leaf_phase": first_downstream_leaf.get("leaf_requested_phase", ""),
            "first_downstream_path_aigs": first_downstream_path.get("path_aigs", ""),
            "first_downstream_path_phases": first_downstream_path.get("path_phases", ""),
        })
    write_csv(
        RESULTS / "demand_path_summary.csv",
        rows_out,
        [
            "benchmark", "watch_aig", "downstream_aig", "target_requests",
            "target_phase0", "target_phase1", "target_via_downstream",
            "leaf_demands", "leaf_phase0", "leaf_phase1", "leaf_via_downstream",
            "phase0_actions", "phase0_direct_or_cached_rows",
            "phase0_inverter_from_opposite_rows", "downstream_parent_phases",
            "downstream_leaf_requested_phases", "output_cos_requesting_watch",
            "output_cos_via_downstream", "first_downstream_parent_gate",
            "first_downstream_leaf_phase", "first_downstream_path_aigs",
            "first_downstream_path_phases",
        ],
    )


def write_parent_cut_summary(stats_by_name, multi_stats_by_name):
    rows_out = []
    for bench in BENCHMARKS:
        stat_rows = [row for row in stats_by_name.get("parent_cut_stats", []) if row["benchmark"] == bench]
        stat = stat_rows[0] if stat_rows else {}
        cuts = [row for row in multi_stats_by_name.get("parent_cut", []) if row["benchmark"] == bench]
        phase0 = next((row for row in cuts if row.get("phase") == "0"), {})
        phase1 = next((row for row in cuts if row.get("phase") == "1"), {})
        rows_out.append({
            "benchmark": bench,
            "child_aig": stat.get("child_aig", phase0.get("child_aig", phase1.get("child_aig", ""))),
            "parent_aig": stat.get("parent_aig", phase0.get("parent_aig", phase1.get("parent_aig", ""))),
            "requests": stat.get("requests", ""),
            "rows": stat.get("rows", ""),
            "phase0_has_cut": stat.get("phase0_has_cut", phase0.get("has_cut", "")),
            "phase1_has_cut": stat.get("phase1_has_cut", phase1.get("has_cut", "")),
            "child_phase0_count": stat.get("child_phase0", ""),
            "child_phase1_count": stat.get("child_phase1", ""),
            "child_missing": stat.get("child_missing", ""),
            "phase0_gate": phase0.get("gate", ""),
            "phase0_child_requested_phase": phase0.get("child_requested_phase", ""),
            "phase0_child_leaf_index": phase0.get("child_leaf_index", ""),
            "phase1_gate": phase1.get("gate", ""),
            "phase1_child_requested_phase": phase1.get("child_requested_phase", ""),
            "phase1_child_leaf_index": phase1.get("child_leaf_index", ""),
            "parent_child_requested_phases": _join_sorted_values(cuts, "child_requested_phase"),
        })
    write_csv(
        RESULTS / "parent_cut_summary.csv",
        rows_out,
        [
            "benchmark", "child_aig", "parent_aig", "requests", "rows",
            "phase0_has_cut", "phase1_has_cut", "child_phase0_count",
            "child_phase1_count", "child_missing", "phase0_gate",
            "phase0_child_requested_phase", "phase0_child_leaf_index",
            "phase1_gate", "phase1_child_requested_phase",
            "phase1_child_leaf_index", "parent_child_requested_phases",
        ],
    )
...
def main():
    ...
    write_reconstruct_summary(stats_by_name, multi_stats_by_name)
    write_demand_path_summary(stats_by_name, multi_stats_by_name)
    write_parent_cut_summary(stats_by_name, multi_stats_by_name)
