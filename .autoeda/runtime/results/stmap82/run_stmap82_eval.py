...
RESULTS = ROOT / ".autoeda" / "runtime" / "results" / "stmap82"
ABC = ROOT / "abc"
LIB = "7nm_lvt_ff.lib"
COMMAND = "stmap82"
BENCHMARKS = [
...
STMAP_LINE_RE = re.compile(r"^stmap82 ([^:]+):\s*(.*)$")
...
def parse_stmap82_rows(bench, text):
    rows_by_kind = {}
...
def run_flow(kind, bench):
    ...
    return {
        ...
        "diagnostics": parse_stmap82_rows(bench, text) if kind == "candidate" else {},
