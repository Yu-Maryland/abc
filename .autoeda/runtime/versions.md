...
- hypothesis: Keep the `stmap48` first-pass SCL pressure selector and the `stmap45` pressure-agreement mapper mode, but use an intermediate high-pressure final remap gain of `0.90 * base_gain` (`225` by default) instead of the `stmap48` `0.80 * base_gain` (`200`). This should reduce the severe `syn2` area cost while preserving or improving the high-pressure timing win, and leave low/moderate-pressure benchmarks on the validated `Gain = 250` path.
- motivation: `stmap48` solved the global-regression problem by selecting `Gain = 200` only for the severe `syn2` pressure case, but it still spent `+1942.29` area versus baseline on `syn2`. Preliminary `syn2` probes showed `stmap47 -G 225` at `479.50 ps` / `22232.05`, better than the `stmap48` selected-low-gain result of `484.80 ps` / `23238.89`; `-G 237.5` reduced area further but gave back timing, while `-G 212.5` regressed delay.
- command name: `stmap49`
- files changed:
  - `src/base/abci/abc.c`
  - `src/base/abci/abcStmap_49.c`
  - `src/base/abci/module.make`
  - `abclib.dsp`
  - `.autoeda/runtime/results/stmap49/*`
  - `.autoeda/runtime/versions.md`
  - `.autoeda/runtime/campaign_state.json`
  - `.autoeda/runtime/last_prompt.txt`
- algorithm summary: `stmap49` is a numbered two-pass `stmap` command. It clones the `stmap48` SCL load/max-cap collection, dense sink-pressure feedback, and high-pressure selector, then remaps through `Map_Stmap45SetSclLoadFeedback()` with mapper mode `46`. The only intended algorithmic change is the selected high-pressure gain: when feedback severity is at least `0.850` and sink-pressure entries are at least `8000`, `stmap49` uses `0.90 * base_gain`; otherwise it keeps `base_gain`. Candidate logs print `stmap49 blended-gain` and `stmap49 mapper-mode` diagnostics. Existing `map` and `stmap0` through `stmap48` remain on their prior command paths and mode numbers.
- validation run:
  - build: `make ABC_USE_NO_READLINE=1` passed and produced `./abc`. Log: `.autoeda/runtime/results/stmap49/build.log`.
  - help: `./abc -c "stmap49 -h"` printed command usage with adaptive blended-gain two-pass mapping. Log: `.autoeda/runtime/results/stmap49/help.log`.
  - smoke: `read_lib 7nm_lvt_ff.lib; read benchmarks/i10.aig; resyn; resyn2; dch -v; stmap49; topo; buffer; upsize -v; dnsize -v; stime` passed with final delay `198.64 ps` and area `1303.10`. Log: `.autoeda/runtime/results/stmap49/smoke_i10.log`.
  - full evaluation artifacts: `.autoeda/runtime/results/stmap49/summary.json`, `metrics.csv`, `comparison.csv`, `blended_gain_stats.csv`, `mapper_mode_stats.csv`, guard/near-miss/early-seed diagnostics, sink-pressure and feedback diagnostics, penalty diagnostics, raw baseline/candidate logs, CEC temporaries and logs, review prompts, review logs, review summary, artifact check, and version-log check.
- benchmark results:
...
- blended-gain and sink-pressure diagnostics:
  - blended gain selected `250.00` for `i10`, `ode`, and `or1200`, and selected `225.00` only for `syn2`.
  - first-pass feedback severity and pressure population were unchanged from `stmap48`: `i10` severity `0.000`, entries `262`; `ode` severity `0.627`, entries `4816`; `or1200` severity `0.733`, entries `4362`; `syn2` severity `0.884`, entries `8980`.
  - `benchmarks/syn2.abc.blif`: tracked AIG `5548`, mapped node `5656`, raw pressure ratio `2.265`, sink-pressure ratio `1.443`, eight moderate pressure blocks, and two strong pressure blocks.
  - Compared with `stmap48`, `stmap49` preserves the same QoR points for `i10`, `ode`, and `or1200`; on `syn2`, it improves delay from `484.80 ps` to `479.50 ps` and reduces area from `23238.89` to `22232.05`.
- correctness results:
  - build passed.
  - numbered implementation exists: `src/base/abci/abcStmap_49.c`.
  - numbered command exists and is registered as `stmap49`.
  - command help passed.
  - smoke flow passed.
  - benchmark metrics passed for all four required designs.
  - blended-gain, mapper-mode, sink-pressure, feedback, guard, early-seed, near-miss, moderate-penalty, and strong-penalty diagnostic parsing passed and is recorded under `.autoeda/runtime/results/stmap49/`.
  - CEC passed for all four required designs using original and candidate strashed AIG temporaries under `.autoeda/runtime/results/stmap49/`.
  - artifact check passed and is recorded in `.autoeda/runtime/results/stmap49/artifact_check.log`.
- review pass 1:
  - configured prompt-form review was attempted with `codex exec review --uncommitted ... "<prompt>"`; the installed Codex CLI rejected the positional prompt with `--uncommitted`. The failure is logged in `.autoeda/runtime/results/stmap49/review_pass1_positional.log`.
  - supported configured-tool invocation completed with the same prompt via stdin: `codex exec review --uncommitted --model gpt-5.5 -c model_reasoning_effort="xhigh" -c service_tier="fast" --dangerously-bypass-approvals-and-sandbox`; log: `.autoeda/runtime/results/stmap49/review_pass1.log`.
  - accepted source findings: none.
- review pass 2:
  - configured prompt-form review was attempted and rejected for the same local CLI argument conflict; the failure is logged in `.autoeda/runtime/results/stmap49/review_pass2_positional.log`.
  - supported configured-tool invocation completed with the same prompt via stdin; log: `.autoeda/runtime/results/stmap49/review_pass2.log`.
  - accepted source findings: none.
- rejected findings: none.
- open findings: none after campaign-state finalization.
...
- hypothesis: Keep the `stmap50` first-pass SCL pressure selector and the `stmap45` pressure-agreement mapper mode, but use a single midpoint high-pressure final remap gain of `0.925 * base_gain` (`231.25` by default). This tests whether the severe-pressure `syn2` point lies between the `stmap49` `225.00` timing-heavy branch and the `stmap50` `237.50` area-saving branch.
- motivation: `stmap50` reduced `syn2` area versus `stmap49` but gave back too much delay. The previous next-step recommendation called for a narrower selected gain such as `231.25`; `stmap51` isolates that gain choice while preserving the existing SCL feedback, sink-pressure collection, and mapper mode.
- command name: `stmap51`
- files changed:
  - `src/base/abci/abc.c`
  - `src/base/abci/abcStmap_51.c`
  - `src/base/abci/module.make`
  - `abclib.dsp`
  - `.autoeda/runtime/results/stmap51/*`
  - `.autoeda/runtime/versions.md`
  - `.autoeda/runtime/campaign_state.json`
  - `.autoeda/runtime/last_prompt.txt`
- algorithm summary: `stmap51` is a numbered two-pass `stmap` command. It clones the `stmap50` SCL load/max-cap collection and dense sink-pressure feedback, then remaps through `Map_Stmap45SetSclLoadFeedback()` with mapper mode `46`. The only intended algorithmic change is the high-pressure selected gain: when feedback severity is at least `0.850` and sink-pressure entries are at least `8000`, it selects `0.925 * base_gain`; otherwise it keeps `base_gain`. Candidate logs print `stmap51 midpoint-gain` and `stmap51 mapper-mode` diagnostics. Existing `map` and `stmap0` through `stmap50` remain on their prior command paths and mode numbers.
- validation run:
  - build: `make ABC_USE_NO_READLINE=1` passed and produced `./abc`. Log: `.autoeda/runtime/results/stmap51/build.log`.
  - help: `./abc -c "stmap51 -h"` printed command usage with midpoint-gain two-pass mapping. Log: `.autoeda/runtime/results/stmap51/help.log`.
  - smoke: `read_lib 7nm_lvt_ff.lib; read benchmarks/i10.aig; resyn; resyn2; dch -v; stmap51; topo; buffer; upsize -v; dnsize -v; stime` passed with final delay `198.64 ps` and area `1303.10`. Log: `.autoeda/runtime/results/stmap51/smoke_i10.log`.
  - full evaluation artifacts: `.autoeda/runtime/results/stmap51/summary.json`, `metrics.csv`, `comparison.csv`, `midpoint_gain_stats.csv`, `mapper_mode_stats.csv`, guard/near-miss/early-seed diagnostics, sink-pressure and feedback diagnostics, penalty diagnostics, raw baseline/candidate logs, CEC temporaries and logs, review prompts, review logs, review summary, artifact check, and version-log check.
- benchmark results:
  - `benchmarks/i10.aig`: baseline delay `207.24 ps`, area `1263.44`; candidate delay `198.64 ps`, area `1303.10`; delay delta `-8.60 ps` (`-4.15%`), area delta `+39.66` (`+3.14%`); selected gain `250.00`; feedback severity `0.000`; sink-pressure entries `262`.
  - `benchmarks/ode.abc.blif`: baseline delay `531.32 ps`, area `12491.21`; candidate delay `522.05 ps`, area `11334.38`; delay delta `-9.27 ps` (`-1.74%`), area delta `-1156.83` (`-9.26%`); selected gain `250.00`; feedback severity `0.627`; sink-pressure entries `4816`.
  - `benchmarks/or1200.abc.blif`: baseline delay `587.08 ps`, area `4798.34`; candidate delay `561.55 ps`, area `4895.15`; delay delta `-25.53 ps` (`-4.35%`), area delta `+96.81` (`+2.02%`); selected gain `250.00`; feedback severity `0.733`; sink-pressure entries `4362`.
  - `benchmarks/syn2.abc.blif`: baseline delay `508.82 ps`, area `21296.60`; candidate delay `505.86 ps`, area `22328.39`; delay delta `-2.96 ps` (`-0.58%`), area delta `+1031.79` (`+4.84%`); selected gain `231.25`; feedback severity `0.884`; sink-pressure entries `8980`.
- midpoint-gain and sink-pressure diagnostics:
  - midpoint gain selected `250.00` for `i10`, `ode`, and `or1200`, and selected `231.25` only for `syn2`.
  - first-pass feedback severity and pressure population were: `i10` severity `0.000`, entries `262`; `ode` severity `0.627`, entries `4816`; `or1200` severity `0.733`, entries `4362`; `syn2` severity `0.884`, entries `8980`.
  - `benchmarks/syn2.abc.blif`: moderate pressure blocks increased to `8`; strong pressure blocks were `0`.
  - Compared with `stmap50`, `stmap51` preserves the same QoR points for `i10`, `ode`, and `or1200`. On `syn2`, it worsens delay from `486.74 ps` to `505.86 ps` and increases area from `21808.18` to `22328.39`. Compared with `stmap49`, it is much slower than `479.50 ps` while also slightly larger than `22232.05`. The midpoint gain hypothesis is therefore falsified on the required benchmark set.
- correctness results:
  - build passed.
  - numbered implementation exists: `src/base/abci/abcStmap_51.c`.
  - numbered command exists and is registered as `stmap51`.
  - command help passed.
  - smoke flow passed.
  - benchmark metrics passed for all four required designs.
  - midpoint-gain, mapper-mode, sink-pressure, feedback, guard, early-seed, near-miss, moderate-penalty, and strong-penalty diagnostic parsing passed and is recorded under `.autoeda/runtime/results/stmap51/`.
  - CEC passed for all four required designs using original and candidate strashed AIG temporaries under `.autoeda/runtime/results/stmap51/`.
  - artifact check passed and is recorded in `.autoeda/runtime/results/stmap51/artifact_check.log`.
- review pass 1:
  - configured prompt-form review was attempted with `codex exec review --uncommitted ... "<prompt>"`; the installed Codex CLI rejected the positional prompt with `--uncommitted`. The failure is logged in `.autoeda/runtime/results/stmap51/review_pass1_positional.log`.
  - supported configured-tool invocation completed with the same prompt via stdin: `codex exec review --uncommitted --model gpt-5.5 -c model_reasoning_effort="xhigh" -c service_tier="fast" --dangerously-bypass-approvals-and-sandbox`; log: `.autoeda/runtime/results/stmap51/review_pass1.log`.
  - accepted source findings: none.
- review pass 2:
  - configured prompt-form review was attempted and rejected for the same local CLI argument conflict; the failure is logged in `.autoeda/runtime/results/stmap51/review_pass2_positional.log`.
  - supported configured-tool invocation completed with the same prompt via stdin; log: `.autoeda/runtime/results/stmap51/review_pass2.log`.
  - accepted source findings: none.
- rejected findings: none.
- open findings: none after campaign-state finalization.
- source changes after review: none; only runtime review summary, artifact checks, canonical logging, version checks, and campaign-state finalization were added after the review passes.
...
- hypothesis: Keep the reviewed `stmap54` severe-feedback branch, but cap cut-only one-sided pressure exceptions to at most `1.25` inverter areas of local area-save. This tests whether the one accepted `stmap54` `syn2` exception was worth its area cost.
- motivation: `stmap54` improved `syn2` delay by `0.36 ps` versus `stmap53`, but cost `6.53` extra area and fired only once on the required set. The campaign recommendation was to preserve the reviewed severe branch while adding a small area-ratio or area-save cap around cut-only exceptions.
- command name: `stmap55`
- files changed:
  - `src/base/abci/abc.c`
  ...
  - `src/map/mapper/mapperInt.h`
  - `abclib.dsp`
  - `.autoeda/runtime/results/stmap55/*`
  - `.autoeda/runtime/versions.md`
  - `.autoeda/runtime/campaign_state.json`
  - `.autoeda/runtime/last_prompt.txt`
- algorithm summary: `stmap55` is a numbered two-pass `stmap` command. It clones the `stmap54` first-pass SCL load/max-cap collection and dense sink-pressure feedback, selects the blended gain only under the same severe feedback conditions, and performs the final remap with mapper mode `56`. Mode `56` preserves the reviewed `stmap53` pressure-near branch through `Map_MatchStmap54ModeratePenaltyFactor()`, preserves the `stmap54` cut-only candidate test, then admits a cut-only exception only when `AreaSave <= 1.25 * inverter_area + epsilon`. Cap-blocked cut-only candidates are reported separately and fall back to the normal strict area gate. Existing `map` and `stmap0` through `stmap54` remain on their prior command paths and mode numbers.
- validation run:
  - build: `make ABC_USE_NO_READLINE=1` passed and produced `./abc`. Log: `.autoeda/runtime/results/stmap55/build.log`.
  - help: `./abc -c "stmap55 -h"` printed command usage with the area-capped cut-only mode. Log: `.autoeda/runtime/results/stmap55/help.log`.
  - smoke: `read_lib 7nm_lvt_ff.lib; read benchmarks/i10.aig; resyn; resyn2; dch -v; stmap55; topo; buffer; upsize -v; dnsize -v; stime` passed with final delay `198.64 ps` and area `1303.10`. Log: `.autoeda/runtime/results/stmap55/smoke_i10.log`.
  - full evaluation artifacts: `.autoeda/runtime/results/stmap55/summary.json`, `metrics.csv`, `comparison.csv`, `feedback_stats.csv`, `mapper_mode_stats.csv`, guard/near-miss/early-seed diagnostics, sink-pressure diagnostics, penalty diagnostics, raw baseline/candidate logs, CEC temporaries and logs, review prompt, review logs, review summary, artifact check, and version-log check.
- benchmark results:
  - `benchmarks/i10.aig`: baseline delay `207.24 ps`, area `1263.44`; candidate delay `198.64 ps`, area `1303.10`; delay delta `-8.60 ps` (`-4.15%`), area delta `+39.66` (`+3.14%`); selected gain `250.00`; feedback severity `0.000`; sink-pressure entries `262`.
  - `benchmarks/ode.abc.blif`: baseline delay `531.32 ps`, area `12491.21`; candidate delay `522.05 ps`, area `11334.38`; delay delta `-9.27 ps` (`-1.74%`), area delta `-1156.83` (`-9.26%`); selected gain `250.00`; feedback severity `0.627`; sink-pressure entries `4816`.
  - `benchmarks/or1200.abc.blif`: baseline delay `587.08 ps`, area `4798.34`; candidate delay `561.55 ps`, area `4895.15`; delay delta `-25.53 ps` (`-4.35%`), area delta `+96.81` (`+2.02%`); selected gain `250.00`; feedback severity `0.733`; sink-pressure entries `4362`.
  - `benchmarks/syn2.abc.blif`: baseline delay `508.82 ps`, area `21296.60`; candidate delay `479.50 ps`, area `22232.05`; delay delta `-29.32 ps` (`-5.76%`), area delta `+935.45` (`+4.39%`); selected gain `225.00`; feedback severity `0.884`; sink-pressure entries `8980`.
- cut-only and severe-pressure diagnostics:
  - mapper mode `56` was used for all candidate benchmark runs; final blended gain was `225.00` only on `syn2`.
  - first-pass feedback severity and pressure population were: `i10` severity `0.000`, entries `262`; `ode` severity `0.627`, entries `4816`; `or1200` severity `0.733`, entries `4362`; `syn2` severity `0.884`, entries `8980`.
  - `benchmarks/syn2.abc.blif`: the prior `stmap54` cut-only seed at mapper node `23628` / AIG ID `27645` was blocked by the new cap. It had slack `6.719986`, area-save `0.930000`, area-save cap `0.875000`, arrival-delta `-4.100006`, arrival-gain-margin `1.510000`, cut sink-pressure ratio `1.794`, node sink-pressure ratio `0.000`, and SCL feedback `0.884`.
  - `benchmarks/syn2.abc.blif`: pressure-near exception seeds/blocks `0/0`, cut-only exception seeds/blocks `0/0`, cut-only area-cap blocks `1`, moderate penalty seeds/blocks `0/1`, strong penalty seeds/blocks `0/2`, near-miss count `18`, and guard middle-relief count `1`.
  - Required-set QoR matches the lower-area `stmap53` point on `syn2`: delay is `479.50 ps` and area is `22232.05`. Relative to `stmap54`, the cap saves `6.53` area but gives back `0.36 ps` delay.
- correctness results:
  - build passed.
  - numbered implementation exists: `src/base/abci/abcStmap_55.c`.
  - numbered command exists and is registered as `stmap55`.
  - command help passed.
  - smoke flow passed.
  - benchmark metrics passed for all four required designs.
  - mapper-mode, sink-pressure, feedback, guard, early-seed, near-miss, pressure-near exception, cut-only exception, cut-only area-cap block, moderate-penalty, and strong-penalty diagnostic parsing passed and is recorded under `.autoeda/runtime/results/stmap55/`.
  - CEC passed for all four required designs using original and candidate strashed AIG temporaries under `.autoeda/runtime/results/stmap55/`.
  - artifact check passed and is recorded in `.autoeda/runtime/results/stmap55/artifact_check.log`.
  - review summary is recorded in `.autoeda/runtime/results/stmap55/review_summary.md`.
- review pass 1:
  - configured prompt-form review was attempted with `codex exec review --uncommitted ... "<prompt>"`; the installed Codex CLI rejected the positional prompt with `--uncommitted`. The failure is logged in `.autoeda/runtime/results/stmap55/review_pass1_positional.log`.
  - supported configured-tool invocation completed with the same prompt via stdin: `codex exec review --uncommitted --model gpt-5.5 -c model_reasoning_effort="xhigh" -c service_tier="fast" --dangerously-bypass-approvals-and-sandbox`; log: `.autoeda/runtime/results/stmap55/review_pass1.log`.
  - accepted source findings: none. Pass 1 reported no actionable correctness regressions in the final uncommitted source changes.
- rejected findings: none.
- open findings: none after pass 1.
- source changes after final review: none; only runtime review summary, artifact checks, canonical logging, version checks, campaign-state finalization, and git bookkeeping were added after the clean final review pass.
- commit: local commit to be created with message `stmap55: cap cut-only pressure exception area`; the exact hash is intentionally left to Git history rather than embedded in this self-referential log record.
- push: enabled by full-campaign `git.yaml`; push is to be performed after the local commit.
- next-step recommendation: The simple area-save cap confirms the `stmap54` one-sided seed was a delay/area tradeoff rather than free QoR. For `stmap56`, prefer a reconstruction-consistent pressure-transfer experiment that can turn cut-only pressure into paired evidence, or a very narrow cap sweep around `1.35x` inverter area to see whether the single `syn2` delay gain can be recovered with a bounded area budget.


## version56 / stmap56

- hypothesis: Keep the reviewed `stmap54`/`stmap55` severe-feedback cut-only pressure exception, but relax the cut-only local area-save cap from `1.25` to `1.35` inverter areas in a new mapper mode. This tests whether the single useful `syn2` timing gain can be recovered with a still explicit area budget.
- motivation: `stmap55` proved that the simple `1.25x` cap saved the `stmap54` area cost but also gave back the only extra `syn2` timing gain. The prior recommendation allowed a narrow cap sweep around `1.35x` inverter area as a controlled boundary experiment.
- command name: `stmap56`
- files changed:
  - `src/base/abci/abc.c`
  ...
  - `src/map/mapper/mapperInt.h`
  - `abclib.dsp`
  - `.autoeda/runtime/results/stmap56/*`
  - `.autoeda/runtime/versions.md`
  - `.autoeda/runtime/campaign_state.json`
  - `.autoeda/runtime/last_prompt.txt`
- algorithm summary: `stmap56` is a numbered two-pass `stmap` command. It clones the `stmap55` first-pass SCL load/max-cap collection and dense sink-pressure feedback, selects the blended gain only under the same severe feedback conditions, and performs the final remap with mapper mode `57`. Mode `57` preserves the reviewed `stmap53` pressure-near branch through the `stmap54` helper, preserves the `stmap54` cut-only candidate test, then admits a cut-only exception only when `AreaSave <= 1.35 * inverter_area + epsilon`. Cap-blocked cut-only candidates remain separately reported and fall back to the normal strict area gate. Existing `map` and `stmap0` through `stmap55` remain on their prior command paths and mode numbers.
- validation run:
  - build: `make ABC_USE_NO_READLINE=1` passed and produced `./abc`. Log: `.autoeda/runtime/results/stmap56/build.log`.
  - help: `./abc -c "stmap56 -h"` printed command usage with the relaxed area-capped cut-only mode. Log: `.autoeda/runtime/results/stmap56/help.log`.
  - smoke: `read_lib 7nm_lvt_ff.lib; read benchmarks/i10.aig; resyn; resyn2; dch -v; stmap56; topo; buffer; upsize -v; dnsize -v; stime` passed with final delay `198.64 ps` and area `1303.10`. Log: `.autoeda/runtime/results/stmap56/smoke_i10.log`.
  - full evaluation artifacts: `.autoeda/runtime/results/stmap56/summary.json`, `metrics.csv`, `comparison.csv`, `feedback_stats.csv`, `mapper_mode_stats.csv`, guard/near-miss/early-seed diagnostics, sink-pressure diagnostics, penalty diagnostics, raw baseline/candidate logs, CEC temporaries and logs, review prompts, review logs, review summary, artifact check, and version-log check.
- benchmark results:
  - `benchmarks/i10.aig`: baseline delay `207.24 ps`, area `1263.44`; candidate delay `198.64 ps`, area `1303.10`; delay delta `-8.60 ps` (`-4.15%`), area delta `+39.66` (`+3.14%`); selected gain `250.00`; feedback severity `0.000`; sink-pressure entries `262`.
  - `benchmarks/ode.abc.blif`: baseline delay `531.32 ps`, area `12491.21`; candidate delay `522.05 ps`, area `11334.38`; delay delta `-9.27 ps` (`-1.74%`), area delta `-1156.83` (`-9.26%`); selected gain `250.00`; feedback severity `0.627`; sink-pressure entries `4816`.
  - `benchmarks/or1200.abc.blif`: baseline delay `587.08 ps`, area `4798.34`; candidate delay `561.55 ps`, area `4895.15`; delay delta `-25.53 ps` (`-4.35%`), area delta `+96.81` (`+2.02%`); selected gain `250.00`; feedback severity `0.733`; sink-pressure entries `4362`.
  - `benchmarks/syn2.abc.blif`: baseline delay `508.82 ps`, area `21296.60`; candidate delay `479.14 ps`, area `22238.58`; delay delta `-29.68 ps` (`-5.83%`), area delta `+941.98` (`+4.42%`); selected gain `225.00`; feedback severity `0.884`; sink-pressure entries `8980`.
- cut-only and severe-pressure diagnostics:
  - mapper mode `57` was used for all candidate benchmark runs; final blended gain was `225.00` only on `syn2`; the cut-only area-save cap was reported as `1.35x-inverter`.
  - first-pass feedback severity and pressure population were: `i10` severity `0.000`, entries `262`; `ode` severity `0.627`, entries `4816`; `or1200` severity `0.733`, entries `4362`; `syn2` severity `0.884`, entries `8980`.
  - `benchmarks/syn2.abc.blif`: the prior one-sided cut-only seed at mapper node `23628` / AIG ID `27645` was admitted by the relaxed cap. It had slack `6.719986`, area-save `0.930000`, area-save cap `0.945000`, arrival-delta `-4.100006`, arrival-gain-margin `1.510000`, cut sink-pressure ratio `1.794`, node sink-pressure ratio `0.000`, and SCL feedback `0.884`.
  - `benchmarks/syn2.abc.blif`: pressure-near exception seeds/blocks `0/0`, cut-only exception seeds/blocks `1/0`, cut-only area-cap blocks `0`, moderate penalty seeds/blocks `0/1`, strong penalty seeds/blocks `0/2`, near-miss count `18`, and guard middle-relief count `1`.
  - Required-set QoR matches `stmap54` on `syn2`: delay is `479.14 ps` and area is `22238.58`. Relative to `stmap55`, the relaxed cap recovers `0.36 ps` delay but spends `6.53` area; other required benchmark QoR points are unchanged from `stmap55`.
- correctness results:
  - build passed.
  - numbered implementation exists: `src/base/abci/abcStmap_56.c`.
  - numbered command exists and is registered as `stmap56`.
  - command help passed.
  - smoke flow passed.
  - benchmark metrics passed for all four required designs.
  - mapper-mode, sink-pressure, feedback, guard, early-seed, near-miss, pressure-near exception, cut-only exception, cut-only area-cap block, moderate-penalty, and strong-penalty diagnostic parsing passed and is recorded under `.autoeda/runtime/results/stmap56/`.
  - CEC passed for all four required designs using original and candidate strashed AIG temporaries under `.autoeda/runtime/results/stmap56/`.
  - artifact check passed and is recorded in `.autoeda/runtime/results/stmap56/artifact_check.log`.
  - review summary is recorded in `.autoeda/runtime/results/stmap56/review_summary.md`.
- review pass 1:
  - configured prompt-form review was attempted with `codex exec review --uncommitted ... "<prompt>"`; the installed Codex CLI rejected the positional prompt with `--uncommitted`. The failure is logged in `.autoeda/runtime/results/stmap56/review_pass1_positional.log`.
  - supported configured-tool invocation completed with the same prompt via stdin: `codex exec review --uncommitted --model gpt-5.5 -c model_reasoning_effort="xhigh" -c service_tier="fast" --dangerously-bypass-approvals-and-sandbox`; log: `.autoeda/runtime/results/stmap56/review_pass1.log`.
  - accepted source findings: none. Pass 1 reported no actionable correctness issues in the current source changes.
- review pass 2:
  - configured prompt-form review was attempted and rejected for the same local CLI argument conflict; the failure is logged in `.autoeda/runtime/results/stmap56/review_pass2_positional.log`.
  - supported configured-tool invocation completed with the same prompt via stdin; log: `.autoeda/runtime/results/stmap56/review_pass2.log`.
...
- hypothesis: Feed the reviewed mapper mode `57` a bounded transferred-pressure table instead of raw pressure. Severe-feedback designs lift sink-weighted pressure toward raw pressure with a `0.72` weight and cap the final selected ratio at `2.10`, while non-severe designs keep the sink-weighted handoff. This tests whether bounded raw transfer can avoid the `stmap57` over-amplification while preserving useful severe-pressure evidence.
- motivation: `stmap57` showed that direct raw pressure was too strong for the useful `syn2` cut-only case: it moved the cut pressure above the reviewed exception band and fell back to the lower-area, slower point. The prior recommendation was to use a bounded transferred-pressure ratio capped near the `1.60` to `2.20` band.
- command name: `stmap58`
- files changed:
  - `src/base/abci/abc.c`
  - `src/base/abci/abcStmap_58.c`
  - `src/base/abci/module.make`
  - `abclib.dsp`
  - `.autoeda/runtime/results/stmap58/*`
  - `.autoeda/runtime/versions.md`
  - `.autoeda/runtime/campaign_state.json`
  - `.autoeda/runtime/last_prompt.txt`
- algorithm summary: `stmap58` clones the `stmap57` two-pass command structure and preserves classic `map` plus `stmap0` through `stmap57`. It first maps with the established feedback collection path, computes the same severe-feedback gain selector, and then remaps through the reviewed mapper mode `57`. The new behavior is the second-pass pressure handoff: `Abc_Stmap58BuildBoundedPressures()` feeds sink-weighted pressure for non-severe cases, and for severe cases uses `max(capped_sink, min(1.0 + 0.72 * (raw - 1.0), 2.10))` for each AIG pressure entry. Pass-1 review tightened this so the final selected ratio is capped even when the selected value came from sink fallback. Existing `map` and older `stmap` commands remain on their prior paths and mode numbers.
- validation run:
  - build: `make ABC_USE_NO_READLINE=1` passed and produced `./abc`. Log: `.autoeda/runtime/results/stmap58/build.log`.
  - help: `./abc -c "stmap58 -h"` printed command usage for bounded-pressure-transfer mode. Log: `.autoeda/runtime/results/stmap58/help.log`.
  - smoke: `read_lib 7nm_lvt_ff.lib; read benchmarks/i10.aig; resyn; resyn2; dch -v; stmap58; topo; buffer; upsize -v; dnsize -v; stime` passed with final delay `198.64 ps` and area `1303.10`. Log: `.autoeda/runtime/results/stmap58/smoke_i10.log`.
  - full evaluation artifacts: `.autoeda/runtime/results/stmap58/summary.json`, `metrics.csv`, `comparison.csv`, `feedback_stats.csv`, `bounded_pressure_stats.csv`, `blended_gain_stats.csv`, `mapper_mode_stats.csv`, `sink_pressure_stats.csv`, inherited guard/near-miss/early-seed/penalty diagnostics, raw baseline/candidate logs, CEC temporaries and logs, review prompts, review logs, review summary, artifact check, and version-log check.
- benchmark results:
...
- bounded-pressure diagnostics:
  ...
  - first-pass feedback severity and pressure population were: `i10` severity `0.000`, entries `262`; `ode` severity `0.627`, entries `4816`; `or1200` severity `0.733`, entries `4362`; `syn2` severity `0.884`, entries `8980`.
  - `syn2` bounded transfer diagnostics: bounded entries `8980`, lifted entries `3540`, capped entries `6316`, raw-only entries `0`, sink-fallback entries `5440`, tracked raw/sink/bounded ratios `2.265` / `1.443` / `1.911`.
  - inherited mode `57` diagnostics on `syn2`: pressure-near exception seeds/blocks `0/0`, cut-only exception seeds/blocks `0/0`, cut-only area-cap blocks `0`, moderate penalty seeds/blocks `0/1`, strong penalty seeds/blocks `0/2`.
  - Relative to `stmap57`, `stmap58` is slower on `syn2` by `0.28 ps` but saves `28.46` area. Relative to `stmap56`, it is slower by `0.64 ps` and saves `34.99` area. The hypothesis is partially falsified: bounding prevents raw over-amplification, but it does not recover the useful cut-only seed.
- correctness results:
  - build passed.
  - numbered implementation exists: `src/base/abci/abcStmap_58.c`.
  - numbered command exists and is registered as `stmap58`.
  - command help passed.
  - smoke flow passed.
  - benchmark metrics passed for all four required designs.
  - feedback, bounded-pressure, blended-gain, mapper-mode, sink-pressure, inherited guard, inherited early-seed, inherited near-miss, and inherited penalty diagnostic parsing passed and is recorded under `.autoeda/runtime/results/stmap58/`.
  - CEC passed for all four required designs using original and candidate strashed AIG temporaries under `.autoeda/runtime/results/stmap58/`.
  - artifact check passed and is recorded in `.autoeda/runtime/results/stmap58/artifact_check.log`.
  - version-log check passed and is recorded in `.autoeda/runtime/results/stmap58/version_log_check.log`.
  - review summary is recorded in `.autoeda/runtime/results/stmap58/review_summary.md`.
- review pass 1:
  - configured prompt-form review was attempted with `codex exec review --uncommitted ... "<prompt>"`; the installed Codex CLI rejected the positional prompt with `--uncommitted`. The failure is logged in `.autoeda/runtime/results/stmap58/review_pass1_positional.log`.
  - supported configured-tool invocation completed with the same prompt via stdin: `codex exec review --uncommitted --model gpt-5.5 -c model_reasoning_effort="xhigh" -c service_tier="fast" --dangerously-bypass-approvals-and-sandbox`; log: `.autoeda/runtime/results/stmap58/review_pass1.log`.
  - accepted source finding: cap severe sink fallback ratios before feeding the bounded table to mode `57`. `Abc_Stmap58BuildBoundedPressures()` now caps the final selected ratio under severe transfer. Revalidated with build, help, smoke, full metrics, bounded-pressure diagnostics, and CEC.
- review pass 2:
  - configured prompt-form review was attempted and rejected for the same local CLI argument conflict; the failure is logged in `.autoeda/runtime/results/stmap58/review_pass2_positional.log`.
  - supported configured-tool invocation completed with the same prompt via stdin; log: `.autoeda/runtime/results/stmap58/review_pass2.log`.
  - accepted source findings: none. Pass 2 reported no actionable correctness issues in the final uncommitted changes.
- rejected findings: none.
- open findings: none after pass 2.
- source changes after final review: none; only runtime review summary, artifact checks, canonical logging, version checks, campaign-state finalization, and git bookkeeping were added after the clean final review pass.
...
- hypothesis: Keep the reviewed `stmap61` bounded-pressure two-pass flow, but during the final feedback remap test the mode-57 cut-only pressure exception before the moderate-candidate gate. This tests whether the `stmap61`-diagnosed severe `syn2` node `23628` event can be admitted when primitive pressure, slack, arrival-strength, and local area-cap gates pass.
- motivation: `stmap61` showed the tracked `syn2` event was blocked by the moderate-candidate ordering rather than by primitive cut-only eligibility, pressure entries, pressure banding, local slack, arrival strength, or area cap. The prior recommendation was to make one narrow ordering change and keep the tracked diagnostic active.
- command name: `stmap62`
- files changed:
  - `src/base/abci/abc.c`
  ...
  - `src/map/mapper/mapperMatch.c`
  - `abclib.dsp`
  - `.autoeda/runtime/results/stmap62/*`
  - `.autoeda/runtime/versions.md`
  - `.autoeda/runtime/campaign_state.json`
  - `.autoeda/runtime/last_prompt.txt`
- algorithm summary: `stmap62` clones the `stmap61` command-level bounded-pressure handoff and preserves classic `map` plus `stmap0` through `stmap61`. It still performs a first pass through the established SCL load/max-cap collector, builds the same bounded pressure table, and remaps through reviewed mapper mode `57`. The intentional source change is command-scoped: `Map_Stmap62SetCutOnlyOrdering(1)` is enabled only around the final `stmap62` remap, allowing `Map_MatchStmap54ModeratePenaltyFactor()` to set the cut-only exception before `Map_MatchIsStmap28ModeratePenaltyCandidate()` rejects the candidate. The flag is initialized off before command execution and reset immediately after the final remap. Existing commands remain on their prior paths.
- validation run:
  - build: `make ABC_USE_NO_READLINE=1` passed and produced `./abc`. Logs: `.autoeda/runtime/results/stmap62/build.log` and post-review metadata rerun `.autoeda/runtime/results/stmap62/build_rerun_after_header.log`.
  - help: `./abc -c "stmap62 -h"` printed command usage. Logs: `.autoeda/runtime/results/stmap62/help.log` and `.autoeda/runtime/results/stmap62/help_rerun_after_header.log`.
  - smoke: `read_lib 7nm_lvt_ff.lib; read benchmarks/i10.aig; resyn; resyn2; dch -v; stmap62; topo; buffer; upsize -v; dnsize -v; stime` passed with final delay `198.64 ps` and area `1303.10`. Logs: `.autoeda/runtime/results/stmap62/smoke_i10.log` and post-review metadata rerun `.autoeda/runtime/results/stmap62/smoke_i10_rerun_after_header.log`.
  - full evaluation artifacts: `.autoeda/runtime/results/stmap62/summary.json`, `metrics.csv`, `comparison.csv`, feedback/bounded-pressure/blended-gain/mapper-mode/sink-pressure diagnostics, inherited guard/near-miss/early-seed/penalty diagnostics, cut-only gate diagnostics, near-miss cut-leaf diagnostics, raw baseline/candidate logs, CEC temporaries and logs, review prompts, review logs, review summary, artifact check, and version-log check.
- benchmark results:
  - `benchmarks/i10.aig`: baseline delay `207.24 ps`, area `1263.44`; candidate delay `198.64 ps`, area `1303.10`; delay delta `-8.60 ps` (`-4.15%`), area delta `+39.66` (`+3.14%`); selected gain `250.00`; pressure feed `sink-fallback`.
  - `benchmarks/ode.abc.blif`: baseline delay `531.32 ps`, area `12491.21`; candidate delay `522.05 ps`, area `11334.38`; delay delta `-9.27 ps` (`-1.74%`), area delta `-1156.83` (`-9.26%`); selected gain `250.00`; pressure feed `sink-fallback`.
  - `benchmarks/or1200.abc.blif`: baseline delay `587.08 ps`, area `4798.34`; candidate delay `561.55 ps`, area `4895.15`; delay delta `-25.53 ps` (`-4.35%`), area delta `+96.81` (`+2.02%`); selected gain `250.00`; pressure feed `sink-fallback`.
  - `benchmarks/syn2.abc.blif`: baseline delay `508.82 ps`, area `21296.60`; candidate delay `482.04 ps`, area `22180.50`; delay delta `-26.78 ps` (`-5.26%`), area delta `+883.90` (`+4.15%`); selected gain `225.00`; pressure feed `bounded-raw`.
- cut-only ordering diagnostics:
  - mapper mode `57` was used for all candidate benchmark runs with `cut-only-before-moderate = 1`; only `syn2` opened severe bounded-raw transfer.
  - `syn2` first-pass feedback severity was `0.884` with `8980` raw pressure entries and bounded transfer active.
  - `syn2` penalty stats recorded `2` cut-only exception seeds, `0` cut-only exception blocks, `0` cut-only area-cap blocks, `1` moderate-penalty block, and `2` strong-penalty blocks.
  - candidate log confirms one admitted cut-only exception is the tracked mapper node `23628` / AIG ID `27645`, with node pressure `0.000`, cut pressure `2.100`, slack `6.719986`, area save `0.930000`, area cap `0.945000`, arrival delta `-12.100006`, and SCL feedback `0.884`.
  - The hypothesis is behaviorally confirmed but QoR-negative relative to `stmap61`: the ordering change admits the tracked cut-only seed, but final `syn2` delay worsens from `479.78 ps` in `stmap61` to `482.04 ps` in `stmap62`. The version still improves delay versus classic `map` on all four required benchmarks.
- correctness results:
  - build passed.
  - numbered implementation exists: `src/base/abci/abcStmap_62.c`.
  - numbered command exists and is registered as `stmap62`.
  - command help passed.
  - smoke flow passed.
  - benchmark metrics passed for all four required designs.
  - feedback, bounded-pressure, blended-gain, mapper-mode, sink-pressure, inherited guard, inherited early-seed, inherited near-miss, inherited penalty, cut-only gate, and near-miss cut-leaf diagnostic parsing passed and is recorded under `.autoeda/runtime/results/stmap62/`.
  - CEC passed for all four required designs using original and candidate strashed AIG temporaries under `.autoeda/runtime/results/stmap62/`.
  - artifact check passed and is recorded in `.autoeda/runtime/results/stmap62/artifact_check.log`.
  - version-log check passed and is recorded in `.autoeda/runtime/results/stmap62/version_log_check.log`.
  - review summary is recorded in `.autoeda/runtime/results/stmap62/review_summary.md`.
- review pass 1:
  - configured prompt-form review was attempted with `codex exec review --uncommitted ... "<prompt>"`; the installed Codex CLI rejected the positional prompt with `--uncommitted`. The failure is logged in `.autoeda/runtime/results/stmap62/review_pass1_positional.log`.
  - supported configured-tool invocation completed with the same prompt via stdin: `codex exec review --uncommitted --model gpt-5.5 -c model_reasoning_effort="xhigh" -c service_tier="fast" --dangerously-bypass-approvals-and-sandbox`; log: `.autoeda/runtime/results/stmap62/review_pass1.log`.
  - accepted source findings: none. Pass 1 reported no actionable correctness issues.
- review pass 2:
  - before pass 2, a non-semantic copied header was corrected from `abcStmap_61.c` to `abcStmap_62.c` and revalidated with build, help, and smoke.
  - configured prompt-form review was attempted and rejected for the same local CLI argument conflict; the failure is logged in `.autoeda/runtime/results/stmap62/review_pass2_positional.log`.
  - supported configured-tool invocation completed with the same prompt via stdin; log: `.autoeda/runtime/results/stmap62/review_pass2.log`.
  - accepted source findings: none. Pass 2 reported no actionable correctness issues and noted the command registration/build wiring and scoped mapper-flag reset.
- rejected findings: none.
- open findings: none after pass 2.
- source changes after final review: none; only runtime review summary, artifact checks, canonical logging, version checks, campaign-state finalization, and git bookkeeping were added after the clean final review pass.
...
- hypothesis: Keep the reviewed `stmap65` mapping policy unchanged and record the mode-57 blocked strong-pressure candidates as downstream `stime` witnesses. This tests whether area-margin-rejected strong-pressure choices, especially those with high load-drive ratio, are actually loaded or timing-critical after `topo`, `buffer`, `upsize`, and `dnsize`.
- motivation: `stmap67` showed the only near-threshold load-drop witness remained low-criticality downstream, so lowering the strong-node cut-only threshold is unsupported. The next useful evidence source is another pressure-transfer class: strong-pressure candidates that are currently rejected by the area margin.
- command name: `stmap68`
- files changed:
  - `src/base/abci/abc.c`
  - `src/base/abci/abcStmap_68.c`
  - `src/base/abci/module.make`
  - `src/map/mapper/mapperMatch.c`
  - `abclib.dsp`
  - `.autoeda/runtime/results/stmap68/*`
  - `.autoeda/runtime/versions.md`
  - `.autoeda/runtime/campaign_state.json`
  - `.autoeda/runtime/last_prompt.txt`
- algorithm summary: `stmap68` is a diagnostic-only wrapper over `stmap65`. It enables `Map_Stmap68SetBlockedStrongWitnessDiag(1)` only while delegating to `Abc_CommandStmap65`, then resets the flag and scopes the existing final-witness printer to the resulting mapped network with label `stmap68`. The mapper policy is unchanged: final remap still uses mode `57`, bounded SCL pressure transfer, selected-gain policy, the `1.25` strong-node load-drop guard, and the inherited `1.35x` inverter area-save cap. The new mapper hook records blocked `stmap56` strong-penalty events as witness candidates without changing their acceptance/rejection decision; duplicate AIG/phase sources are logged but only the first unique witness is stored.
- validation run:
  - build: `make ABC_USE_NO_READLINE=1` passed and produced `./abc`; log: `.autoeda/runtime/results/stmap68/build.log`.
  - help: `./abc -c "stmap68 -h"` printed usage text; log: `.autoeda/runtime/results/stmap68/help.log`.
  - smoke: `read_lib 7nm_lvt_ff.lib; read benchmarks/i10.aig; resyn; resyn2; dch -v; stmap68; topo; buffer; upsize -v; dnsize -v; stime` passed with final delay `198.64 ps` and area `1303.10`; log: `.autoeda/runtime/results/stmap68/smoke_i10.log`.
  - full evaluation artifacts: `.autoeda/runtime/results/stmap68/summary.json`, `metrics.csv`, `comparison.csv`, feedback/bounded-pressure/blended-gain/mapper-mode/sink-pressure diagnostics, inherited guard/near-miss/early-seed/penalty diagnostics, cut-only gate diagnostics, near-miss leaf diagnostics, new `blocked_strong_witness_source.csv`, new `final_witness.csv`, raw baseline/candidate logs, CEC temporaries and logs, review logs, review summary, artifact check, and version-log check.
- benchmark results:
  - `benchmarks/i10.aig`: baseline delay `207.24 ps`, area `1263.44`; candidate delay `198.64 ps`, area `1303.10`; delay delta `-8.60 ps` (`-4.15%`), area delta `+39.66` (`+3.14%`); selected gain `250.00`; pressure feed `sink-fallback`.
  - `benchmarks/ode.abc.blif`: baseline delay `531.32 ps`, area `12491.21`; candidate delay `522.05 ps`, area `11334.38`; delay delta `-9.27 ps` (`-1.74%`), area delta `-1156.83` (`-9.26%`); selected gain `250.00`; pressure feed `sink-fallback`.
  - `benchmarks/or1200.abc.blif`: baseline delay `587.08 ps`, area `4798.34`; candidate delay `561.55 ps`, area `4895.15`; delay delta `-25.53 ps` (`-4.35%`), area delta `+96.81` (`+2.02%`); selected gain `250.00`; pressure feed `sink-fallback`.
  - `benchmarks/syn2.abc.blif`: baseline delay `508.82 ps`, area `21296.60`; candidate delay `479.78 ps`, area `22203.59`; delay delta `-29.04 ps` (`-5.71%`), area delta `+906.99` (`+4.26%`); selected gain `225.00`; pressure feed `bounded-raw`.
- blocked strong-pressure witness diagnostic results:
  - `i10`, `ode`, and `or1200` had `0` blocked strong-pressure witness sources.
  - `syn2` had `2` blocked strong-pressure source events on mapper node `2318`, AIG ID `5548`, phase `0`; the first stored event had load-drive ratio `6.750`, node pressure `1.911`, cut pressure `1.712`, area save `0.930000`, area margin `1.119019`, arrival delta `-16.480011`, and penalty factor `0.599`.
  - The duplicate source for the same AIG/phase had load-drive ratio `5.500`, cut pressure `1.552`, area margin `0.999807`, and was logged as `duplicate = 1` without replacing the stored witness.
  - The stored witness matched final node `5784` after downstream flow with gate `INVxp33_ASAP7_75t_L`, final fanouts `4`, final load `1.652`, final max cap `11.520`, final load ratio `0.143`, final criticality `0.285`, and final slack `61.763306`.
  - Final-witness summary was `witnesses = 1`, `matched = 1`, `criticality-ge-0p50 = 0`; this does not support relaxing the strong-pressure area margin yet.
- correctness results:
  - build passed.
  - numbered implementation exists: `src/base/abci/abcStmap_68.c`.
  - numbered command exists and is registered as `stmap68`.
  - command help passed.
  - smoke flow passed.
  - benchmark metrics passed for all four required designs.
  - blocked-strong witness diagnostics, final-witness diagnostics, feedback, bounded-pressure, blended-gain, mapper-mode, sink-pressure, inherited guard, inherited early-seed, inherited near-miss, inherited penalty, cut-only gate, and near-miss leaf diagnostic parsing passed and is recorded under `.autoeda/runtime/results/stmap68/`.
  - CEC passed for all four required designs using original and candidate strashed AIG temporaries under `.autoeda/runtime/results/stmap68/`.
  - artifact check passed and is recorded in `.autoeda/runtime/results/stmap68/artifact_check.log`.
  - version-log check passed and is recorded in `.autoeda/runtime/results/stmap68/version_log_check.log`.
  - review summary is recorded in `.autoeda/runtime/results/stmap68/review_summary.md`.
- review pass 1:
  - configured prompt-form review was attempted with `codex exec review --uncommitted ... "<prompt>"`; the installed Codex CLI rejected the positional prompt with the known `--uncommitted` conflict. The failure is logged in `.autoeda/runtime/results/stmap68/review_pass1_positional.log`.
  - supported configured-tool invocation completed with the same prompt via stdin; log: `.autoeda/runtime/results/stmap68/review_pass1.log`.
  - accepted source findings: none. Pass 1 reported no discrete correctness issues.
- review pass 2:
  - configured prompt-form review was attempted and rejected for the same local CLI argument conflict; the failure is logged in `.autoeda/runtime/results/stmap68/review_pass2_positional.log`.
  - supported configured-tool invocation completed with the same prompt via stdin; log: `.autoeda/runtime/results/stmap68/review_pass2.log`.
  - accepted source findings: none. Pass 2 reported no discrete correctness issues and confirmed the diagnostic path is gated to `stmap68` without changing mapper behavior.
- rejected findings: none.
- open findings: none after pass 2.
- source changes after final review: none; only runtime review summary, artifact checks, canonical logging, version checks, campaign-state finalization, and git bookkeeping were added after the clean final review pass.
- commit: local commit to be created with message `stmap68: trace blocked strong-pressure witnesses`; the exact hash is intentionally left to Git history rather than embedded in this self-referential log record.
- push: enabled by full-campaign `git.yaml`; push is to be performed after the local commit.
- next-step recommendation: For `stmap69`, keep both the strong-node cut-only threshold and the strong-pressure area margin closed. The only blocked strong-pressure witness was visible after downstream timing but still had final load ratio `0.143` and final criticality `0.285`, so a margin-relaxation variant is not yet supported. Next, either trace pressure-near exception candidates or add a stricter diagnostic that only proposes mapper changes for witnesses with final criticality at least `0.50`.


## version69 / stmap69

- hypothesis: Keep the reviewed `stmap65` mapping policy unchanged and record mode-57 pressure-near exception candidates as downstream `stime` witnesses. This tests whether the pressure-near exception class has any final load or criticality evidence before reopening mapper admission rules.
- motivation: `stmap67` and `stmap68` showed the near-threshold load-drop and blocked strong-pressure witnesses that survived downstream were low-criticality. The next conservative diagnostic is to check whether the pressure-near class itself produces any candidates under the current bounded-pressure policy.
- command name: `stmap69`
- files changed:
  - `src/base/abci/abc.c`
  - `src/base/abci/abcStmap_69.c`
  - `src/base/abci/module.make`
  - `src/map/mapper/mapperMatch.c`
  - `abclib.dsp`
  - `.autoeda/runtime/results/stmap69/*`
  - `.autoeda/runtime/versions.md`
  - `.autoeda/runtime/campaign_state.json`
  - `.autoeda/runtime/last_prompt.txt`
- algorithm summary: `stmap69` is a diagnostic-only wrapper over `stmap65`. It enables `Map_Stmap69SetPressureNearWitnessDiag(1)` only while delegating to `Abc_CommandStmap65`, resets the flag immediately afterward, and scopes the existing final-witness printer to the resulting mapped network with label `stmap69` when mapping succeeds. The mapper policy is unchanged: final remap still uses mode `57`, bounded SCL pressure transfer, selected-gain policy, the `1.25` strong-node load-drop guard, and the inherited `1.35x` inverter area-save cap. The new mapper hook records `stmap56` mode-57 pressure-near seed/block events as witness candidates without changing their acceptance decision; duplicate AIG/phase sources are logged but only the first unique witness is stored.
- validation run:
  - build: `make ABC_USE_NO_READLINE=1` passed and produced `./abc`; log: `.autoeda/runtime/results/stmap69/build.log`.
  - help: `./abc -c "stmap69 -h"` printed usage text; log: `.autoeda/runtime/results/stmap69/help.log`.
  - smoke: `read_lib 7nm_lvt_ff.lib; read benchmarks/i10.aig; resyn; resyn2; dch -v; stmap69; topo; buffer; upsize -v; dnsize -v; stime` passed with final delay `198.64 ps` and area `1303.10`; log: `.autoeda/runtime/results/stmap69/smoke_i10.log`.
  - full evaluation artifacts: `.autoeda/runtime/results/stmap69/summary.json`, `metrics.csv`, `comparison.csv`, feedback/bounded-pressure/blended-gain/mapper-mode/sink-pressure diagnostics, inherited guard/near-miss/early-seed/penalty diagnostics, cut-only gate diagnostics, near-miss leaf diagnostics, new `pressure_near_witness_source.csv`, new `final_witness.csv`, raw baseline/candidate logs, CEC temporaries and logs, review logs, review summary, artifact check, and version-log check.
- benchmark results:
  - `benchmarks/i10.aig`: baseline delay `207.24 ps`, area `1263.44`; candidate delay `198.64 ps`, area `1303.10`; delay delta `-8.60 ps` (`-4.15%`), area delta `+39.66` (`+3.14%`); selected gain `250.00`; pressure feed `sink-fallback`.
  - `benchmarks/ode.abc.blif`: baseline delay `531.32 ps`, area `12491.21`; candidate delay `522.05 ps`, area `11334.38`; delay delta `-9.27 ps` (`-1.74%`), area delta `-1156.83` (`-9.26%`); selected gain `250.00`; pressure feed `sink-fallback`.
  - `benchmarks/or1200.abc.blif`: baseline delay `587.08 ps`, area `4798.34`; candidate delay `561.55 ps`, area `4895.15`; delay delta `-25.53 ps` (`-4.35%`), area delta `+96.81` (`+2.02%`); selected gain `250.00`; pressure feed `sink-fallback`.
  - `benchmarks/syn2.abc.blif`: baseline delay `508.82 ps`, area `21296.60`; candidate delay `479.78 ps`, area `22203.59`; delay delta `-29.04 ps` (`-5.71%`), area delta `+906.99` (`+4.26%`); selected gain `225.00`; pressure feed `bounded-raw`.
- pressure-near witness diagnostic results:
  - all four required benchmarks had `0` pressure-near witness source rows; `pressure_near_witness_source.csv`, `final_witness.csv`, `final_witness_stats.csv`, and `final_witness_summary.csv` intentionally contain headers only.
  - inherited `stmap56` penalty stats recorded `0` pressure-near exception seeds and `0` pressure-near exception blocks on every required benchmark.
  - `syn2` still showed `1` moderate-penalty block and `2` strong-penalty blocks under the inherited mode-57 policy, matching the prior pressure classes and confirming that pressure-near is not the active blocker for the current required-benchmark set.
- correctness results:
  - build passed.
  - numbered implementation exists: `src/base/abci/abcStmap_69.c`.
  - numbered command exists and is registered as `stmap69`.
  - command help passed.
  - smoke flow passed.
  - benchmark metrics passed for all four required designs.
  - pressure-near witness diagnostics, final-witness header artifacts, feedback, bounded-pressure, blended-gain, mapper-mode, sink-pressure, inherited guard, inherited early-seed, inherited near-miss, inherited penalty, cut-only gate, and near-miss leaf diagnostic parsing passed and is recorded under `.autoeda/runtime/results/stmap69/`.
  - CEC passed for all four required designs using original and candidate strashed AIG temporaries under `.autoeda/runtime/results/stmap69/`.
  - artifact check passed and is recorded in `.autoeda/runtime/results/stmap69/artifact_check.log`.
  - version-log check passed and is recorded in `.autoeda/runtime/results/stmap69/version_log_check.log`.
  - review summary is recorded in `.autoeda/runtime/results/stmap69/review_summary.md`.
- review pass 1:
  - configured prompt-form review was attempted with `codex exec review --uncommitted ... "<prompt>"`; the installed Codex CLI rejected the positional prompt with the known `--uncommitted` conflict. The failure is logged in `.autoeda/runtime/results/stmap69/review_pass1_positional.log`.
  - supported configured-tool invocation completed with the same prompt via stdin; log: `.autoeda/runtime/results/stmap69/review_pass1.log`.
  - accepted source findings: none. Pass 1 reported no discrete correctness issues in command registration, diagnostic scoping, witness recording, or build integration.
- review pass 2:
  - configured prompt-form review was attempted and rejected for the same local CLI argument conflict; the failure is logged in `.autoeda/runtime/results/stmap69/review_pass2_positional.log`.
  - supported configured-tool invocation completed with the same prompt via stdin; log: `.autoeda/runtime/results/stmap69/review_pass2.log`.
  - accepted source findings: none. Pass 2 reported no actionable correctness issues and confirmed the command is scoped and does not change inherited `stmap65` mapping behavior.
- rejected findings: none.
- open findings: none after pass 2.
- source changes after final review: none; only runtime review summary, artifact checks, canonical logging, version checks, campaign-state finalization, and git bookkeeping were added after the clean final review pass.
...
- hypothesis: Keep the reviewed `stmap65` mapping policy unchanged and record mode-57 moderate-penalty block candidates as downstream `stime` witnesses. Reopen the moderate-block admission class only if a source survives `topo; buffer; upsize; dnsize; stime` with final criticality at least `0.50`.
- motivation: `stmap70` combined the already-closed near-strong-node, blocked strong-pressure, and pressure-near witness classes and found no final-critical witnesses. The remaining pressure-transfer blocker class with measurable activity on the required benchmarks is the moderate-penalty block class, so `stmap71` checks it directly before any policy change.
- command name: `stmap71`
- files changed:
  - `src/base/abci/abc.c`
  - `src/base/abci/abcStmap_71.c`
  - `src/base/abci/module.make`
  - `src/map/mapper/mapperMatch.c`
  - `abclib.dsp`
  - `.autoeda/runtime/results/stmap71/*`
  - `.autoeda/runtime/versions.md`
  - `.autoeda/runtime/campaign_state.json`
  - `.autoeda/runtime/last_prompt.txt`
- algorithm summary: `stmap71` is a diagnostic-only wrapper over `Abc_CommandStmap65`. It enables the new command-scoped `Map_Stmap71SetModeratePenaltyWitnessDiag(1)` flag only while the delegated `stmap65` command runs, disables it immediately afterward, and scopes the existing final-witness printer to the resulting mapped network with label `stmap71`. The mapper policy is unchanged: final remap still uses mapper mode `57`, bounded SCL pressure transfer, selected-gain policy, the `1.25` strong-node load-drop guard, and the inherited `1.35x` inverter area-save cap. The new mapper hook records `stmap56` mode-57 moderate-penalty block sources as witness candidates without changing their acceptance decision; duplicate AIG/phase sources are logged but only the first unique witness is stored.
- validation run:
  - build: `make ABC_USE_NO_READLINE=1` passed and produced `./abc`. Log: `.autoeda/runtime/results/stmap71/build.log`.
  - help: `./abc -c "stmap71 -h"` printed `stmap71` usage. Log: `.autoeda/runtime/results/stmap71/help.log`.
  - smoke: `read_lib 7nm_lvt_ff.lib; read benchmarks/i10.aig; resyn; resyn2; dch -v; stmap71; topo; buffer; upsize -v; dnsize -v; stime` passed with final delay `198.64 ps` and area `1303.10`. Log: `.autoeda/runtime/results/stmap71/smoke_i10.log`.
  - full evaluation artifacts: `.autoeda/runtime/results/stmap71/summary.json`, `metrics.csv`, `comparison.csv`, feedback/bounded-pressure/blended-gain/mapper-mode/sink-pressure diagnostics, inherited guard/near-miss/early-seed/penalty diagnostics, cut-only gate diagnostics, near-miss leaf diagnostics, new `moderate_penalty_witness_source.csv`, `final_witness.csv`, raw baseline/candidate logs, CEC temporaries and logs, review logs, review summary, artifact check, and version-log check.
- benchmark results:
  - `benchmarks/i10.aig`: baseline delay `207.24 ps`, area `1263.44`; candidate delay `198.64 ps`, area `1303.10`; delay delta `-8.60 ps` (`-4.15%`), area delta `+39.66` (`+3.14%`); selected gain `250.00`; pressure feed `sink-fallback`.
  - `benchmarks/ode.abc.blif`: baseline delay `531.32 ps`, area `12491.21`; candidate delay `522.05 ps`, area `11334.38`; delay delta `-9.27 ps` (`-1.74%`), area delta `-1156.83` (`-9.26%`); selected gain `250.00`; pressure feed `sink-fallback`.
  - `benchmarks/or1200.abc.blif`: baseline delay `587.08 ps`, area `4798.34`; candidate delay `561.55 ps`, area `4895.15`; delay delta `-25.53 ps` (`-4.35%`), area delta `+96.81` (`+2.02%`); selected gain `250.00`; pressure feed `sink-fallback`.
  - `benchmarks/syn2.abc.blif`: baseline delay `508.82 ps`, area `21296.60`; candidate delay `479.78 ps`, area `22203.59`; delay delta `-29.04 ps` (`-5.71%`), area delta `+906.99` (`+4.26%`); selected gain `225.00`; pressure feed `bounded-raw`.
- moderate-penalty witness diagnostic results:
  - `i10`, `ode`, and `or1200` had `0` moderate-penalty witness source rows.
  - `syn2` had `1` moderate-penalty witness source row: mapper node `12167`, AIG ID `13915`, phase `1`, node pressure `1.165`, cut pressure `2.100`, slack `6.739990`, area save `0.930000`, area margin `1.400000`, arrival delta `-4.720001`, arrival-gain margin `1.510000`, area cap `0`, and feedback severity `0.884`.
  - The `syn2` source matched final node `11282` after downstream flow with gate `O2A1O1Ixp33_ASAP7_75t_L`, final fanouts `1`, final load `0.446`, final max cap `11.520`, final load ratio `0.039`, final criticality `0.272`, and final slack `62.842468`.
  - Final-witness summary was `witnesses = 1`, `matched = 1`, `criticality-ge-0p50 = 0`; this does not support reopening the moderate-penalty block class.
- correctness results:
  - build passed.
  - numbered implementation exists: `src/base/abci/abcStmap_71.c`.
  - numbered command exists and is registered as `stmap71`.
  - command help passed.
  - smoke flow passed.
  - benchmark metrics passed for all four required designs.
  - moderate-penalty witness diagnostics, final-witness diagnostics, feedback, bounded-pressure, blended-gain, mapper-mode, sink-pressure, inherited guard, early-seed, near-miss, penalty, cut-only gate, and near-miss leaf diagnostic parsing passed and is recorded under `.autoeda/runtime/results/stmap71/`.
  - CEC passed for all four required designs using original and candidate strashed AIG temporaries under `.autoeda/runtime/results/stmap71/`.
  - artifact check passed and is recorded in `.autoeda/runtime/results/stmap71/artifact_check.log`.
  - version-log check passed and is recorded in `.autoeda/runtime/results/stmap71/version_log_check.log`.
  - review summary is recorded in `.autoeda/runtime/results/stmap71/review_summary.md`.
- review pass 1:
  - configured prompt-form review was attempted with `codex exec review --uncommitted ... "<prompt>"`; the installed Codex CLI rejected the positional prompt with the known `--uncommitted` conflict. The failure is logged in `.autoeda/runtime/results/stmap71/review_pass1_positional.log`.
  - supported configured-tool invocation completed with the same prompt via stdin; log: `.autoeda/runtime/results/stmap71/review_pass1.log`.
  - accepted source findings: none. Pass 1 reported no discrete correctness issue in command registration, diagnostic scoping, witness recording, or build integration.
- review pass 2:
  - configured prompt-form review was attempted and rejected for the same local CLI argument conflict; the failure is logged in `.autoeda/runtime/results/stmap71/review_pass2_positional.log`.
  - supported configured-tool invocation completed with the same prompt via stdin; log: `.autoeda/runtime/results/stmap71/review_pass2.log`.
  - accepted source findings: none. Pass 2 reported no actionable correctness issue in the final uncommitted state.
- rejected findings: none.
- open findings: none after pass 2.
- source changes after final review: none; only runtime review summary, artifact checks, canonical logging, version checks, campaign-state finalization, and git bookkeeping were added after the clean final review pass.
- commit: local commit to be created with message `stmap71: trace moderate-penalty witnesses`; the exact hash is intentionally left to Git history rather than embedded in this self-referential log record.
- push: enabled by full-campaign `git.yaml`; push is to be performed after the local commit.
- next-step recommendation: For `stmap72`, keep the moderate-penalty block closed on the current benchmark evidence because its only matched downstream witness had final criticality `0.272`, below the `0.50` gate. The next useful step is to search for a different blocker class with final-criticality evidence, or to add a diagnostic that ranks blocked candidates by final path proximity rather than pressure class alone before making any mapper policy change.


## version72 / stmap72

- hypothesis: Keep the reviewed `stmap65` mapping policy unchanged, combine all currently closed pressure-transfer blocker witness classes, and rank matched downstream witnesses by final `stime` path proximity. Reopen a mapper admission rule only if the combined witness set exposes final-critical path evidence.
- motivation: `stmap67` through `stmap71` showed isolated near-strong-node, blocked strong-pressure, pressure-near, and moderate-penalty diagnostics. The only matched downstream witnesses were low-criticality. `stmap72` checks the combined population directly and ranks it by final criticality/slack rather than by source class alone.
- command name: `stmap72`
- files changed:
  - `src/base/abci/abc.c`
  - `src/base/abci/abcStmap_72.c`
  - `src/base/abci/module.make`
  - `src/map/mapper/mapperMatch.c`
  - `abclib.dsp`
  - `.autoeda/runtime/results/stmap72/*`
  - `.autoeda/runtime/versions.md`
  - `.autoeda/runtime/campaign_state.json`
  - `.autoeda/runtime/last_prompt.txt`
- algorithm summary: `stmap72` is a diagnostic-only wrapper over `Abc_CommandStmap65`. It enables `Map_Stmap66SetNearStrongNodeLoadDropDiag(1)` and the new command-scoped `Map_Stmap72SetPathProximityWitnessDiag(1)` only while the delegated `stmap65` command runs, disables both flags immediately afterward, and scopes the existing final-witness printer to the resulting mapped network with label `stmap72`. The inherited mapping policy is unchanged: final remap still uses mapper mode `57`, bounded SCL pressure transfer, selected-gain policy, the `1.25` strong-node load-drop guard, and the inherited `1.35x` inverter area-save cap. The new mapper flag lets the existing near-strong-node, blocked strong-pressure, pressure-near, and moderate-penalty witness recorders share one final-witness population under the `stmap72` command.
- validation run:
  - build: `make ABC_USE_NO_READLINE=1` passed and produced `./abc`. Log: `.autoeda/runtime/results/stmap72/build.log`.
  - help: `./abc -c "stmap72 -h"` printed `stmap72` usage. Log: `.autoeda/runtime/results/stmap72/help.log`.
  - smoke: `read_lib 7nm_lvt_ff.lib; read benchmarks/i10.aig; resyn; resyn2; dch -v; stmap72; topo; buffer; upsize -v; dnsize -v; stime` passed with final delay `198.64 ps` and area `1303.10`. Log: `.autoeda/runtime/results/stmap72/smoke_i10.log`.
  - full evaluation artifacts: `.autoeda/runtime/results/stmap72/summary.json`, `metrics.csv`, `comparison.csv`, feedback/bounded-pressure/blended-gain/mapper-mode/sink-pressure diagnostics, inherited guard/near-miss/early-seed/penalty diagnostics, cut-only gate diagnostics, near-miss leaf diagnostics, near-strong-node diagnostics, blocked-strong witness sources, pressure-near witness source headers, moderate-penalty witness sources, combined `final_witness.csv`, new `path_proximity_ranking.csv`, new `path_proximity_summary.csv`, raw baseline/candidate logs, CEC temporaries and logs, review logs, review summary, artifact check, and version-log check.
- benchmark results:
  - `benchmarks/i10.aig`: baseline delay `207.24 ps`, area `1263.44`; candidate delay `198.64 ps`, area `1303.10`; delay delta `-8.60 ps` (`-4.15%`), area delta `+39.66` (`+3.14%`); selected gain `250.00`; pressure feed `sink-fallback`.
  - `benchmarks/ode.abc.blif`: baseline delay `531.32 ps`, area `12491.21`; candidate delay `522.05 ps`, area `11334.38`; delay delta `-9.27 ps` (`-1.74%`), area delta `-1156.83` (`-9.26%`); selected gain `250.00`; pressure feed `sink-fallback`.
  - `benchmarks/or1200.abc.blif`: baseline delay `587.08 ps`, area `4798.34`; candidate delay `561.55 ps`, area `4895.15`; delay delta `-25.53 ps` (`-4.35%`), area delta `+96.81` (`+2.02%`); selected gain `250.00`; pressure feed `sink-fallback`.
  - `benchmarks/syn2.abc.blif`: baseline delay `508.82 ps`, area `21296.60`; candidate delay `479.78 ps`, area `22203.59`; delay delta `-29.04 ps` (`-5.71%`), area delta `+906.99` (`+4.26%`); selected gain `225.00`; pressure feed `bounded-raw`.
- path-proximity witness results:
  - `i10`, `ode`, and `or1200` had no combined closed-class final witnesses.
  - `syn2` had two matched downstream witnesses. Rank 1 was blocked strong-pressure AIG `5548`, phase `0`, final node `5784`, gate `INVxp33_ASAP7_75t_L`, final criticality `0.285`, final slack `61.763306`, final load ratio `0.143`, seed node pressure `1.911`, and seed cut pressure `1.712`.
  - Rank 2 was AIG `13915`, phase `1`, source classes `moderate-penalty+near-strong-node`, final node `11282`, gate `O2A1O1Ixp33_ASAP7_75t_L`, final criticality `0.272`, final slack `62.842468`, final load ratio `0.039`, seed node pressure `1.165`, and seed cut pressure `2.100`.
  - Combined path-proximity summary for `syn2` was `witnesses = 2`, `matched = 2`, `criticality_ge_0p25 = 2`, `criticality_ge_0p35 = 0`, and `criticality_ge_0p50 = 0`; pressure-near still produced no source rows on the required benchmark set.
  - The diagnostic does not support reopening near-strong-node, blocked strong-pressure, pressure-near, or moderate-penalty admission rules on the current evidence.
- correctness results:
  - build passed.
  - numbered implementation exists: `src/base/abci/abcStmap_72.c`.
  - numbered command exists and is registered as `stmap72`.
  - command help passed.
  - smoke flow passed.
  - benchmark metrics passed for all four required designs.
  - path-proximity ranking, combined final-witness diagnostics, near-strong-node diagnostics, blocked strong-pressure diagnostics, pressure-near header artifacts, moderate-penalty diagnostics, feedback, bounded-pressure, blended-gain, mapper-mode, sink-pressure, inherited guard, early-seed, near-miss, penalty, cut-only gate, and near-miss leaf diagnostic parsing passed and is recorded under `.autoeda/runtime/results/stmap72/`.
  - CEC passed for all four required designs using original and candidate strashed AIG temporaries under `.autoeda/runtime/results/stmap72/`.
  - artifact check passed and is recorded in `.autoeda/runtime/results/stmap72/artifact_check.log`.
  - version-log check passed and is recorded in `.autoeda/runtime/results/stmap72/version_log_check.log`.
  - review summary is recorded in `.autoeda/runtime/results/stmap72/review_summary.md`.
- review pass 1:
  - configured prompt-form review was attempted with `codex exec review --uncommitted ... "<prompt>"`; the installed Codex CLI rejected the positional prompt with the known `--uncommitted` conflict. The failure is logged in `.autoeda/runtime/results/stmap72/review_pass1_positional.log`.
  - supported configured-tool invocation completed with the same prompt via stdin; log: `.autoeda/runtime/results/stmap72/review_pass1.log`.
  - accepted source findings: none. Pass 1 reported no discrete correctness issues.
- review pass 2:
  - configured prompt-form review was attempted and rejected for the same local CLI argument conflict; the failure is logged in `.autoeda/runtime/results/stmap72/review_pass2_positional.log`.
  - supported configured-tool invocation completed with the same prompt via stdin; log: `.autoeda/runtime/results/stmap72/review_pass2.log`.
  - accepted findings: metadata finalization findings for the missing `stmap72` version log entry and transient campaign state. Both were addressed by adding this canonical version entry and finalizing `.autoeda/runtime/campaign_state.json`.
- rejected findings: none.
- open findings: none after addressing the accepted metadata findings.
- source changes after final review: none; only runtime review summary, artifact checks, canonical logging, version checks, campaign-state finalization, and git bookkeeping were added after source review.
...
- hypothesis: Keep the reviewed `stmap65` mapping policy unchanged and trace mapper candidate cuts for the watched downstream parent AIG pairs. If non-selected parent candidates exist that request the watched child in phase `0`, then selected parent-cut tracing alone is too late and the next policy target should be parent candidate scoring or retention.
- motivation: `stmap79` showed that the final selected `ode` parent AIG `5229` cuts do not contain an alternative that requests child AIG `5219` phase `0`. The remaining question was whether such alternatives exist during mapper candidate matching but are discarded before final remap reconstruction.
- command name: `stmap80`
- files changed:
  - `src/base/abci/abc.c`
  - `src/base/abci/abcStmap_65.c`
  - `src/base/abci/abcStmap_80.c`
  - `src/base/abci/module.make`
  - `src/map/mapper/mapperMatch.c`
  - `abclib.dsp`
  - `.autoeda/runtime/results/stmap80/*`
  - `.autoeda/runtime/versions.md`
  - `.autoeda/runtime/campaign_state.json`
- algorithm summary: `stmap80` is a diagnostic-only wrapper over `Abc_CommandStmap65`. It configures the same required-benchmark watched child/parent AIG pairs as `stmap79`, reuses selected-match, final-remap reconstruction, demand-path, parent-cut, final-critical, and phase-survival diagnostics under the `stmap80` label, and adds mapper-side candidate-cut tracing inside `Map_MatchNodePhase`. The candidate-cut trace is active only for the configured final mapping pass and watched downstream parent AIG; it records mapper mode, phase, cut ordinal, reason, viability, selected-update status, selected gate, arrival, required time, area flow, previous best, and watched-child leaf/requested phase. The inherited mapping policy, mapper scoring, gate selection, buffering, sizing, and classic `map` behavior are intentionally unchanged.
- validation run:
  - build: `make ABC_USE_NO_READLINE=1` passed and produced `./abc`. Log: `.autoeda/runtime/results/stmap80/build.log`.
  - help: `./abc -c "stmap80 -h"` printed `stmap80` usage. Log: `.autoeda/runtime/results/stmap80/help.log`.
  - smoke: `read_lib 7nm_lvt_ff.lib; read benchmarks/i10.aig; resyn; resyn2; dch -v; stmap80; topo; buffer; upsize -v; dnsize -v; stime` passed with final delay `198.64 ps` and area `1303.10`, and printed candidate-cut, parent-cut, demand-path, reconstruction, selected-match, final-critical, and phase-survival diagnostics. Log: `.autoeda/runtime/results/stmap80/smoke_i10.log`.
  - path-normalization check: `read ./benchmarks/i10.aig; stmap80` selected watch child AIG `855` and downstream parent AIG `861`, but reported zero candidate/reconstruction rows in the shorter direct-read flow. This is recorded as a diagnostic lifecycle check rather than a full-flow evidence source. Log: `.autoeda/runtime/results/stmap80/path_norm_i10.log`.
  - stale-state check: one ABC process ran watched `i10` and then unknown `.autoeda/runtime/results/stmap79/cec_benchmarks_i10_aig_orig.aig`; the unknown network reported watch IDs `-1` and zero selected-match, reconstruction, demand-path, parent-cut, and candidate-cut diagnostics. Log: `.autoeda/runtime/results/stmap80/unknown_aig_stale_check.log`.
  - full evaluation artifacts: `.autoeda/runtime/results/stmap80/summary.json`, `metrics.csv`, `comparison.csv`, `candidate_cut.csv`, `candidate_cut_stats.csv`, `candidate_cut_summary.csv`, `parent_cut.csv`, `parent_cut_stats.csv`, `parent_cut_summary.csv`, `demand_path.csv`, `demand_leaf.csv`, `demand_path_stats.csv`, `reconstruct_stats.csv`, `reconstruct_request.csv`, `reconstruct_emit.csv`, `reconstruct_cache.csv`, `selected_match.csv`, `selected_match_stats.csv`, `phase_survival.csv`, `phase_survival_stats.csv`, `final_critical_lineage.csv`, `final_critical_stats.csv`, `cec_summary.json`, raw baseline/candidate logs, CEC temporaries and logs, review logs, review summary, artifact check, and version-log check.
- benchmark results:
  - `benchmarks/i10.aig`: baseline delay `207.24 ps`, area `1263.44`; candidate delay `198.64 ps`, area `1303.10`; delay delta `-8.60 ps` (`-4.15%`), area delta `+39.66` (`+3.14%`).
  - `benchmarks/ode.abc.blif`: baseline delay `531.32 ps`, area `12491.21`; candidate delay `522.05 ps`, area `11334.38`; delay delta `-9.27 ps` (`-1.74%`), area delta `-1156.83` (`-9.26%`).
  - `benchmarks/or1200.abc.blif`: baseline delay `587.08 ps`, area `4798.34`; candidate delay `561.55 ps`, area `4895.15`; delay delta `-25.53 ps` (`-4.35%`), area delta `+96.81` (`+2.02%`).
...
- candidate-cut diagnostic results:
  - `i10` parent AIG `861` / child AIG `855`: `304` candidate rows, `15` viable rows, `11` accepted updates, `5` viable child-phase-0 rows, `5` viable child-phase-1 rows, and `2` non-selected child-phase-0 rows. Phase-0 minimum arrival was `82.26 ps`; phase-1 minimum arrival was `81.55 ps`.
  - `ode` parent AIG `5229` / child AIG `5219`: `144` candidate rows, `28` viable rows, `8` accepted updates, `7` viable child-phase-0 rows, `6` viable child-phase-1 rows, and `6` non-selected child-phase-0 rows. The selected-cut view from `stmap79` missed these child-phase-0 alternatives because they were rejected before final selected parent cuts.
  - `ode` child-phase-0 parent candidates include `AOI211xp5_ASAP7_75t_L`, `AOI221xp5_ASAP7_75t_L`, and `AOI311xp33_ASAP7_75t_L`. In the phase-aware area-recovery modes, the child-phase-0 candidates are non-selected because the final `NOR2xp33_ASAP7_75t_L` best cut is smaller and, in the final mode, slightly faster (`183.44 ps` best arrival versus `183.98 ps` for the closest child-phase-0 candidate).
  - `or1200` parent AIG `13829` / child AIG `11491`: `140` candidate rows, `17` viable rows, `6` accepted updates, `7` viable child-phase-0 rows, `4` viable child-phase-1 rows, and `4` non-selected child-phase-0 rows.
  - `syn2` parent AIG `18467` / child AIG `18002`: `184` candidate rows, `24` viable rows, `12` accepted updates, `2` viable child-phase-0 rows, `6` viable child-phase-1 rows, and `2` non-selected child-phase-0 rows.
- parent-cut diagnostic results inherited from `stmap79`:
  - `ode` child AIG `5219` / parent AIG `5229`: selected parent phase `0` uses `OAI211xp5_ASAP7_75t_L` and requests child phase `1`; selected parent phase `1` uses `NOR2xp33_ASAP7_75t_L` but does not include child `5219`. Therefore the final selected parent cuts still contain no downstream-parent alternative that requests child `5219` phase `0`.
- correctness results:
  - build passed.
  - numbered implementation exists: `src/base/abci/abcStmap_80.c`.
  - numbered command exists and is registered as `stmap80`.
  - command help passed.
  - smoke flow passed.
  - benchmark metrics passed for all four required designs.
  - candidate-cut, parent-cut, demand-path, final-remap reconstruction, selected-match, final-critical lineage, phase-survival, path-normalization, stale-state, and CEC diagnostic parsing passed and is recorded under `.autoeda/runtime/results/stmap80/`.
  - CEC passed for all four required designs using original and candidate strashed AIG temporaries under `.autoeda/runtime/results/stmap80/`.
  - artifact check passed and is recorded in `.autoeda/runtime/results/stmap80/artifact_check.log`.
  - review summary is recorded in `.autoeda/runtime/results/stmap80/review_summary.md`.
- review pass 1:
  - configured prompt-form review was attempted with `codex exec review --uncommitted ... "<prompt>"`; the installed Codex CLI rejected the positional prompt with the known `--uncommitted` conflict. The failure is logged in `.autoeda/runtime/results/stmap80/review_pass1_positional.log`.
  - supported configured-tool invocation completed with the same prompt via stdin; log: `.autoeda/runtime/results/stmap80/review_pass1.log`.
  - accepted source findings: none. Pass 1 reported that the changes build and the new `stmap80` diagnostics appear isolated from mapper decisions when disabled.
- review pass 2:
  - configured prompt-form review was attempted and rejected for the same local CLI argument conflict; the failure is logged in `.autoeda/runtime/results/stmap80/review_pass2_positional.log`.
  - supported configured-tool invocation completed with the same prompt via stdin; log: `.autoeda/runtime/results/stmap80/review_pass2.log`.
  - accepted source findings: none. Pass 2 reported no discrete correctness, build, or diagnostic issue in the modified or untracked changes.
- rejected findings: none.
- open findings: none after pass 2.
- source changes after final review: none; only runtime review summary, artifact check, canonical logging, version check, campaign-state finalization, and git bookkeeping were added after the clean final source review.
...
- hypothesis: The final surviving `ode` parent AIG `5229` phase `0` path can be explained at mapper-selected-match level by directly tracing the downstream parent rather than the child. If selected-match shows phase `0` has tight positive slack and already consumes child AIG `5219` in phase `1`, then the next useful policy probe should target timing/load on that parent phase rather than the previously forced non-surviving parent phase `1`.
- motivation: `stmap84` proved that final timing survives on child AIG `5219` phase `1` and parent AIG `5229` phase `0`. Earlier selected-match diagnostics watched the child, leaving parent mapper selection details implicit. `stmap85` keeps the strict sticky and dual phase-survival diagnostics but moves selected-match watch to the downstream parent AIG.
- command name: `stmap85`
- files changed:
  - `src/base/abci/abc.c`
  - `src/base/abci/abcStmap_85.c`
  - `src/base/abci/module.make`
  - `abclib.dsp`
  - `.autoeda/runtime/results/stmap85/*`
  - `.autoeda/runtime/versions.md`
  - `.autoeda/runtime/campaign_state.json`
- algorithm summary: `stmap85` is a diagnostic extension of `stmap84`. It keeps the strict `100000.00 ps` sticky timing hold window, the same parent/child cut diagnostics, and the dual child/parent final phase-survival watch. The only command-level diagnostic change is that `Map_Stmap75SetFinalCriticalAigDiag` is configured with `ParentAigId`, so `selected_match.csv` records the downstream parent AIG mapper choices.
- validation run:
  - build, help, smoke, path-normalization, stale-state, and implementation checks passed under `.autoeda/runtime/results/stmap85/`.
  - full evaluation passed for all four required benchmarks, generated selected-match, phase-survival, sticky, parent-cut, candidate-cut, demand, reconstruction, and final-critical artifacts, and reported CEC `4/4`.
- benchmark results:
  - `benchmarks/i10.aig`: candidate delay `198.64 ps`, area `1303.10`; delay delta `-8.60 ps` (`-4.15%`), area delta `+39.66` (`+3.14%`).
  - `benchmarks/ode.abc.blif`: candidate delay `522.05 ps`, area `11334.38`; delay delta `-9.27 ps` (`-1.74%`), area delta `-1156.83` (`-9.26%`).
  - `benchmarks/or1200.abc.blif`: candidate delay `561.55 ps`, area `4895.15`; delay delta `-25.53 ps` (`-4.35%`), area delta `+96.81` (`+2.02%`).
  - `benchmarks/syn2.abc.blif`: candidate delay `479.78 ps`, area `22203.59`; delay delta `-29.04 ps` (`-5.71%`), area delta `+906.99` (`+4.26%`).
- selected-match diagnostic results:
  - Parent selected-match watches were active for all four required benchmarks: `i10` parent `861` with `16` rows, `ode` parent `5229` with `16` rows, `or1200` parent `13829` with `12` rows, and `syn2` parent `18467` with `16` rows.
  - For `ode`, mapper mode `3` parent phase `0` selects `OAI211xp5_ASAP7_75t_L`, arrival `173.970 ps`, required `174.050 ps`, slack `0.080 ps`, area-flow `1.400`, and consumes child AIG `5219`.
  - For `ode`, strict parent phase `1` still selects `AOI221xp5_ASAP7_75t_L`, arrival `184.330 ps`, area-flow `1.630`, and consumes child AIG `5219`, but `stmap84` showed this phase does not survive final timing.
  - Dual phase-survival remains unchanged: `ode` child AIG `5219` survives final timing only in phase `1`, while parent AIG `5229` survives only in phase `0`.
- correctness results:
  - build passed.
  - numbered implementation exists: `src/base/abci/abcStmap_85.c`.
  - numbered command exists and is registered as `stmap85`.
  - command help passed.
  - smoke flow passed.
  - benchmark metrics and CEC passed for all four required designs.
  - downstream-parent selected-match, dual child/parent phase-survival, strict sticky-parent-phase, parent-phase-bias, candidate-cut, parent-cut, demand-path, reconstruction, path-normalization, stale-state, artifact check, and version-log check passed.
- review pass 1:
...
- hypothesis: A per-benchmark final-consumer phase-pair target can align mapper exact-area recovery with the phase pair that actually survives downstream `stime`. If each watched child/parent pair is targeted in the final phase observed in `stmap88`, the mapper should avoid wasting overrides on phase pairs that do not dominate final timing.
- motivation: `stmap88` showed local child phase-speed overrides were unavailable for `ode` and QoR-inert for `or1200`. Its phase-output context showed the final consumer pairs differ by benchmark: `i10` uses child `855` phase `0` into parent `861` phase `1`, `ode` uses child `5219` phase `1` into parent `5229` phase `0`, `or1200` uses child `11491` phase `0` into parent `13829` phase `1`, and `syn2` uses child `18002` phase `1` into parent `18467` phase `1`.
- command name: `stmap89`
- files changed:
  - `src/base/abci/abc.c`
  - `src/base/abci/abcStmap_65.c`
  - `src/base/abci/abcStmap_89.c`
  - `src/base/abci/module.make`
  - `src/map/mapper/mapperMatch.c`
  - `abclib.dsp`
  - `.autoeda/runtime/results/stmap89/*`
  - `.autoeda/runtime/versions.md`
  - `.autoeda/runtime/campaign_state.json`
- algorithm summary: `stmap89` adds `Map_Stmap89SetConsumerPhaseTarget()` in `mapperMatch.c`. The helper is inactive by default and is activated by `stmap65` only around the final remap when explicitly configured. During exact-area modes `2` and `3`, it may override a non-selected parent match when the watched parent is being mapped in the benchmark-specific target parent phase, the cut consumes the watched child in the benchmark-specific target child phase, and the candidate is within `0.30 ps` arrival, `0.80` area-flow premium, and `2.00x` area-ratio windows. Unlike `stmap88`, this command does not configure the fixed `stmap87` parent target or the `stmap88` child-speed target.
- validation run:
  - build: `make ABC_USE_NO_READLINE=1` passed and produced `./abc`. Log: `.autoeda/runtime/results/stmap89/build.log`.
  - help: `./abc -c "stmap89 -h"` printed usage text. Log: `.autoeda/runtime/results/stmap89/help.log`.
  - smoke: `read_lib 7nm_lvt_ff.lib; read benchmarks/i10.aig; resyn; resyn2; dch -v; stmap89; topo; buffer; upsize -v; dnsize -v; stime` passed with final delay `198.64 ps` and area `1303.10`. Log: `.autoeda/runtime/results/stmap89/smoke_i10.log`.
  - path-normalization check: direct `./benchmarks/i10.aig` selected child AIG `855`, parent AIG `861`, target parent phase `1`, target child phase `0`, and zero selected-match rows without a library as expected. Log: `.autoeda/runtime/results/stmap89/path_norm_i10.log`.
  - stale-state check: one ABC process ran watched `i10` and then an unknown network; the second command reported selected-watch count `0`, consumer-phase target disabled, selected-match rows `0`, and consumer-phase-target rows `0`. Log: `.autoeda/runtime/results/stmap89/unknown_aig_stale_check.log`.
  - full evaluation artifacts: `.autoeda/runtime/results/stmap89/summary.json`, `metrics.csv`, `comparison.csv`, `consumer_phase_target.csv`, `consumer_phase_target_stats.csv`, selected-match diagnostics, phase-survival diagnostics, parent/candidate/demand/reconstruction/final-critical diagnostics, CEC logs and temporaries, review logs, review summary, artifact check, and version-log check.
- benchmark results:
  - `benchmarks/i10.aig`: baseline delay `207.24 ps`, area `1263.44`; candidate delay `198.64 ps`, area `1303.10`; delay delta `-8.60 ps` (`-4.15%`), area delta `+39.66` (`+3.14%`).
  - `benchmarks/ode.abc.blif`: baseline delay `531.32 ps`, area `12491.21`; candidate delay `522.05 ps`, area `11334.38`; delay delta `-9.27 ps` (`-1.74%`), area delta `-1156.83` (`-9.26%`).
  - `benchmarks/or1200.abc.blif`: baseline delay `587.08 ps`, area `4798.34`; candidate delay `561.55 ps`, area `4895.15`; delay delta `-25.53 ps` (`-4.35%`), area delta `+96.81` (`+2.02%`).
  - `benchmarks/syn2.abc.blif`: baseline delay `508.82 ps`, area `21296.60`; candidate delay `479.78 ps`, area `22203.59`; delay delta `-29.04 ps` (`-5.71%`), area delta `+906.99` (`+4.26%`).
- consumer-phase-target diagnostic results:
  - `i10`: target parent phase `1`, child phase `0`; `15` rows, `5` target-child hits, `2` eligible overrides, final QoR unchanged.
  - `ode`: target parent phase `0`, child phase `1`; `28` rows, `6` target-child hits, `2` eligible overrides, final QoR unchanged.
  - `or1200`: target parent phase `1`, child phase `0`; `17` rows, `7` target-child hits, `2` eligible overrides, `1` blocked-window row, final QoR unchanged.
  - `syn2`: target parent phase `1`, child phase `1`; `24` rows, `6` target-child hits, `2` eligible overrides, final QoR unchanged.
  - The unchanged QoR across all four benchmarks suggests these parent/child phase-pair overrides are still selecting mapper-equivalent ties that do not alter downstream buffering/sizing-critical structure.
- correctness results:
  - build passed.
  - numbered implementation exists: `src/base/abci/abcStmap_89.c`.
  - numbered command exists and is registered as `stmap89`.
  - command help passed.
  - smoke flow passed.
  - benchmark metrics and CEC passed for all four required designs.
  - consumer-phase-target, selected-match, phase-survival, parent-cut, candidate-cut, demand-path, reconstruction, path-normalization, stale-state, artifact check, and version-log check passed.
- review pass 1:
  - configured prompt-form review was attempted and rejected by the known local Codex CLI `--uncommitted` positional prompt conflict; stdin review completed with exit `0` and no discrete correctness, build, or integration issues.
- review pass 2:
  - configured prompt-form review was attempted and rejected by the same local CLI conflict; stdin review completed with exit `0` and no actionable correctness issues. The review noted the new command is gated, wired into build/registration, and scoped to the final mapping pass.
- rejected findings: none.
- open findings: none after pass 2.
- source changes after final review: none; only runtime review summary, artifact check, canonical logging, version check, campaign-state finalization, and git bookkeeping were added after the clean final source review.
...
- hypothesis: The mapper should bias the AIG/phase that actually survives into the final critical mapped network, not only an upstream consumer relationship. If a same-phase candidate for that final-critical emitted AIG improves the mapper fanout/load-drive proxy within tight timing and area windows, it may produce a downstream-visible `stime` change.
- motivation: `stmap91` forced one phase-flexible exact-area override on `syn2`, but final QoR did not move. Its reconstruction and final-critical diagnostics showed that the relevant final timing points were the child/emitted AIG phases: `i10` AIG `855` phase `0`, `ode` AIG `5219` phase `1`, `or1200` AIG `11491` phase `0`, and `syn2` AIG `18002` phase `1`. `stmap92` targets those survivor phases directly.
- command name: `stmap92`
- files changed:
  - `src/base/abci/abc.c`
  - `src/base/abci/abcStmap_65.c`
  - `src/base/abci/abcStmap_92.c`
  - `src/base/abci/module.make`
  - `src/map/mapper/mapperMatch.c`
  - `abclib.dsp`
  - `.autoeda/runtime/results/stmap92/*`
  - `.autoeda/runtime/versions.md`
  - `.autoeda/runtime/campaign_state.json`
- algorithm summary: `stmap92` adds `Map_Stmap92SetEmittedDriveTarget()` and `Map_Stmap92MaybeTargetEmittedDrive()` in `mapperMatch.c`. The hook is inactive by default and is enabled only by `stmap92` during the `stmap65` final remap. For the benchmark-specific final-critical AIG/phase, it examines modes `1`, `2`, and `3`; a non-selected candidate may override only if it is the target phase, has an existing best match and best cut, is within a `0.75 ps` arrival window, an area-flow premium of `1.20`, and a `2.50x` area-flow ratio, and improves either fanout limit or cut-load/fanout-limit ratio by at least `0.05`. The command keeps reconstruction, demand-path, parent/candidate cut, selected-match, final-critical, and phase-survival diagnostics enabled for the target AIG and downstream AIG.
- validation run:
  - build: `make ABC_USE_NO_READLINE=1` passed and produced `./abc`. Log: `.autoeda/runtime/results/stmap92/build.log`.
  - help: `./abc -c "stmap92 -h"` printed usage text. Log: `.autoeda/runtime/results/stmap92/help.log`.
  - smoke: `read_lib 7nm_lvt_ff.lib; read benchmarks/i10.aig; resyn; resyn2; dch -v; stmap92; topo; buffer; upsize -v; dnsize -v; stime` passed with final delay `198.64 ps` and area `1303.10`. Log: `.autoeda/runtime/results/stmap92/smoke_i10.log`.
  - path-normalization check: direct `./benchmarks/i10.aig` and `./i10.aig` selected target AIG `855`, downstream AIG `861`, target phase `0`, and emitted-drive targeting was active in both normalized names. Log: `.autoeda/runtime/results/stmap92/path_norm_i10.log`.
  - stale-state check: one ABC process ran watched `i10` and then `.autoeda/runtime/results/stmap92/unknown.blif`; the unknown network reported selected-watch count `0`, emitted-drive target disabled, and emitted-drive-target rows `0`. Log: `.autoeda/runtime/results/stmap92/unknown_aig_stale_check.log`.
  - full evaluation artifacts: `.autoeda/runtime/results/stmap92/summary.json`, `metrics.csv`, `comparison.csv`, `emitted_drive_target.csv`, `emitted_drive_target_stats.csv`, selected-match diagnostics, phase-survival diagnostics, parent/candidate/demand/reconstruction/final-critical diagnostics, CEC logs and temporaries, review logs, review summary, artifact check, and version-log check.
- benchmark results:
  - `benchmarks/i10.aig`: baseline delay `207.24 ps`, area `1263.44`; candidate delay `198.64 ps`, area `1303.10`; delay delta `-8.60 ps` (`-4.15%`), area delta `+39.66` (`+3.14%`).
  - `benchmarks/ode.abc.blif`: baseline delay `531.32 ps`, area `12491.21`; candidate delay `522.05 ps`, area `11334.38`; delay delta `-9.27 ps` (`-1.74%`), area delta `-1156.83` (`-9.26%`).
  - `benchmarks/or1200.abc.blif`: baseline delay `587.08 ps`, area `4798.34`; candidate delay `561.55 ps`, area `4895.15`; delay delta `-25.53 ps` (`-4.35%`), area delta `+96.81` (`+2.02%`).
  - `benchmarks/syn2.abc.blif`: baseline delay `508.82 ps`, area `21296.60`; candidate delay `479.78 ps`, area `22203.59`; delay delta `-29.04 ps` (`-5.71%`), area delta `+906.99` (`+4.26%`).
- emitted-drive diagnostic results:
  - `i10`: target AIG `855` phase `0`; `13` rows, `7` target-phase rows, `2` drive-improved rows, `0` eligible overrides, and `0` overrides.
  - `ode`: target AIG `5219` phase `1`; `28` rows, `6` target-phase rows, `1` drive-improved row, `0` eligible overrides, and `0` overrides.
  - `or1200`: target AIG `11491` phase `0`; `50` rows, `26` target-phase rows, `10` drive-improved rows, `2` eligible rows, `2` already-selected rows, and `0` overrides.
  - `syn2`: target AIG `18002` phase `1`; `52` rows, `26` target-phase rows, `1` drive-improved row, `0` eligible overrides, and `0` overrides.
  - Final QoR stayed unchanged. The useful signal is that direct emitted-phase targeting found `or1200` drive-improved and eligible rows, but both were already selected by the mapper. Exact-area alternatives that might change gate class were blocked by area windows, so the policy did not introduce a new emitted gate.
- correctness results:
  - build passed.
  - numbered implementation exists: `src/base/abci/abcStmap_92.c`.
  - numbered command exists and is registered as `stmap92`.
  - command help passed.
  - smoke flow passed.
  - benchmark metrics and CEC passed for all four required designs.
  - emitted-drive-target, selected-match, phase-survival, parent-cut, candidate-cut, demand-path, reconstruction, path-normalization, stale-state, artifact check, and version-log check passed.
- review pass 1:
  - configured prompt-form review was attempted and rejected by the known local Codex CLI `--uncommitted` positional prompt conflict; stdin review completed with exit `0` and no discrete correctness, build integration, stale-state, or behavior-regression issues.
- review pass 2:
  - configured prompt-form review was attempted and rejected by the same local CLI conflict; stdin review completed with exit `0` and no discrete correctness or build integration issues.
- rejected findings: none.
- open findings: none after pass 2.
- source changes after final review: none; only runtime review summary, artifact check, canonical logging, version check, campaign-state finalization, and git bookkeeping were added after the clean final source review.
